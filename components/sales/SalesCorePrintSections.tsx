...
const SalesCorePrintSections: React.FC<SalesCorePrintSectionsProps> = ({
  ...
  const renderPctBreakdown = (viewValue: string, ownValue: string, companyValue: string) => (
    <div className="space-y-1 text-right text-xs text-slate-600">
      <div>
        <span className="font-semibold text-slate-700">View:</span> {viewValue}
      </div>
      <div>
        <span className="font-semibold text-slate-700">Own:</span> {ownValue}
      </div>
      <div>
        <span className="font-semibold text-slate-700">Co:</span> {companyValue}
      </div>
    </div>
  );
              ...
              <table className="w-full table-fixed divide-y divide-slate-200 text-sm">
                <thead className="bg-slate-50">
                  <tr>
                    ...
                    <th className="px-4 py-2 text-right text-xs font-semibold text-slate-500 uppercase">Sales Mix</th>
                    <th className="px-4 py-2 text-right text-xs font-semibold text-slate-500 uppercase">Units Sold</th>
                    <th className="px-4 py-2 text-right text-xs font-semibold text-slate-500 uppercase">Units Mix</th>
                ...
                <tbody className="bg-white divide-y divide-slate-200">
                  ...
                  <tr className="bg-slate-50 font-semibold">
                    ...
                    <td className="px-4 py-2 align-top">
                      {renderPctBreakdown(
                        printTotalsStore.totalRetail > 0 ? "100.0%" : "0.0%",
                        "--",
                    ...
                    <td className="px-4 py-2 align-top">
                      {renderPctBreakdown(
                        printTotalsStore.totalUnits > 0 ? "100.0%" : "0.0%",
                        "--",
