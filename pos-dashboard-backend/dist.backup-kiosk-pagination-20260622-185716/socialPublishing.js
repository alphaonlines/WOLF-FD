...
function normalizeGoogleActionType(input) {
...
function assetPublicUrl(publicBaseUrl, asset) {
    if (!asset)
        return null;
...
async function waitForInstagramContainer(containerId, accessToken) {
    for (let attempt = 0; attempt < 12; attempt += 1) {
        const status = await fetchJson(`https://graph.facebook.com/${META_API_VERSION}/${containerId}?fields=status_code,status&access_token=${encodeURIComponent(accessToken)}`);
...
async function publishToInstagram(account, post, asset, publicBaseUrl) {
    const igId = String(account.external_id || "").trim();
    const accessToken = String(account.access_token || "").trim();
    if (!igId || !accessToken)
        throw new Error("Instagram account is not configured with account ID and access token.");
    if (!asset)
        throw new Error("Instagram publishing requires an uploaded image or video asset.");
    const assetUrl = assetPublicUrl(publicBaseUrl, asset);
    if (!assetUrl)
        throw new Error("Instagram asset URL could not be built.");
    ...
    const caption = [String(post.caption || "").trim(), post.link_url ? String(post.link_url).trim() : ""]
        .filter(Boolean)
        .join("\n\n");
    const mediaPayload = asset.asset_kind === "video"
        ? {
            caption,
            video_url: assetUrl,
            media_type: "REELS",
...
async function publishToGoogle(account, post, asset, publicBaseUrl) {
    ...
    if (!summary)
        throw new Error("Google Business Profile posts require caption or title text.");
    const body = {
        languageCode: "en-US",
        summary,
        topicType,
...
async function publishForPlatform(platform, account, post, asset, publicBaseUrl) {
    let response;
...
function createSocialPublisher({ pool, publicBaseUrl }) {
    async function runJob(jobId) {
      ...
      `, [job.post_id]);
        if (!postResult.rows.length)
            throw new Error("Social post not found for publish job.");
      ...
      `, [job.platform, job.account_id]);
        ...
        if (post.asset_id) {
          ...
          LIMIT 1
        `, [post.asset_id]);
            asset = assetResult.rows.length ? assetResult.rows[0] : null;
        }
...
//# sourceMappingURL=socialPublishing.js.map