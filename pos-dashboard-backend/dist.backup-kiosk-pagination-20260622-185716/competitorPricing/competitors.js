...
exports.lookupFurniture4Less = lookupFurniture4Less;
exports.lookupAshley = lookupAshley;
...
function noMatch(competitor, notes, url = "") {
    return {
        competitor,
        title: "",
        price: "",
...
function buildLookupQuery(row) {
    return [(0, matching_1.strongestSkuToken)(row.sku), row.description].filter(Boolean).join(" ").replace(/\s+/g, " ").trim();
}
...
function extractMarkdownCandidates(markdown, baseUrl, fallbackUrl, fallbackTitle = "") {
    ...
    while ((match = linkRegex.exec(text))) {
        ...
        if (!title || !url || seen.has(`${title}|${url}`))
            continue;
        if (/^(view|next|previous|cart|checkout|account|login|search)$/i.test(title))
            continue;
        seen.add(`${title}|${url}`);
        const start = Math.max(0, match.index - 500);
        const end = Math.min(text.length, linkRegex.lastIndex + 1000);
        const snippet = text.slice(start, end);
        const price = (0, matching_1.parseFirstPrice)(snippet);
        candidates.push({
            title,
            url,
            price,
    ...
    if (!candidates.length) {
        const price = (0, matching_1.parseFirstPrice)(text);
        candidates.push({
...
function scoreCandidate(competitor, row, candidate) {
    const score = (0, matching_1.classifyCompetitorMatch)({
        sourceSku: row.sku,
...
function chooseBest(matches) {
    const sorted = [...matches].sort((a, b) => {
        ...
        if (rankDelta)
            return rankDelta;
        const aPrice = (0, matching_1.priceToNumber)(a.price) ?? Number.POSITIVE_INFINITY;
        const bPrice = (0, matching_1.priceToNumber)(b.price) ?? Number.POSITIVE_INFINITY;
        if (aPrice !== bPrice)
...
async function lookupFurniture4Less(row) {
    const query = buildLookupQuery(row);
    if (!query)
        return noMatch("Furniture4LessNC", ["missing SKU and description"]);
    const searchUrl = `${FURNITURE4LESS_BASE}/search?q=${encodeURIComponent(query)}`;
    const scrape = await (0, firecrawlClient_1.scrapeWithFirecrawl)(searchUrl);
    ...
    if (!scrape.success && !markdown) {
        return noMatch("Furniture4LessNC", [scrape.error || "Furniture4LessNC scrape failed", scrape.statusCode ? `status ${scrape.statusCode}` : ""].filter(Boolean), searchUrl);
    }
...
async function lookupAshley(row) {
    const lookupQuery = buildLookupQuery(row);
    const query = ["site:ashleyfurniture.com", lookupQuery].filter(Boolean).join(" ").trim();
    if (!lookupQuery)
        return noMatch("Ashley", ["missing SKU and description"]);
    const searchResults = await (0, searxClient_1.searchSearx)(query);
    const ashleyUrls = [];
    const seen = new Set();
    for (const result of searchResults) {
        const url = String(result.url || "").trim();
        if (!url || !isAshleyUrl(url) || seen.has(url))
            continue;
        seen.add(url);
        ashleyUrls.push(url);
        if (ashleyUrls.length >= 5)
            break;
    }
    ...
    for (const url of ashleyUrls) {
        const scrape = await (0, firecrawlClient_1.scrapeWithFirecrawl)(url);
        const markdown = scrape.markdown || "";
        ...
        const candidate = {
            ...
            url,
            price: (0, matching_1.parseFirstPrice)(markdown) || (0, matching_1.parseFirstPrice)(sourceSearchResult?.content || ""),
            text,
        ...
        if (!scrape.success && scrape.error)
            candidate.notes.push(scrape.error);
