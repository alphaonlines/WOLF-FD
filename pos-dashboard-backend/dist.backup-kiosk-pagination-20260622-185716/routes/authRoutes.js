...
function buildGoogleRequestProfile(payload, row) {
...
async function verifyGoogleCredential(pool, credential) {
    const config = await (0, appSettings_1.loadGoogleWorkspaceAuthSettings)(pool, {
    ...
    const googleClient = new google_auth_library_1.OAuth2Client(config.googleClientId);
...
async function createAuthSessionForUser({ pool, authSessionDays, createSessionToken, sha256Hex, userId, req, res, setAuthCookie, }) {
...
async function loadUserRecordByEmail(pool, email) {
    return pool.query(`
...
function registerAuthRoutes({ app, pool, authSessionDays, authCookieName, publicAuthPaths, verifyPassword, hashPassword, sha256Hex, createSessionToken, setAuthCookie, clearAuthCookie, parseCookies, currentAuthUserFromReq, loadAuthUserById, buildAuthUser, }) {
    ...
    app.post("/api/auth/change-password", async (req, res) => {
        ...
        if (newPassword.length < 4)
            return res.status(400).json({ ok: false, error: "new password must be at least 4 chars" });
        const currentRow = await pool.query("SELECT id, password_hash FROM users WHERE id = $1 AND active = TRUE LIMIT 1", [Number(user.id)]);
