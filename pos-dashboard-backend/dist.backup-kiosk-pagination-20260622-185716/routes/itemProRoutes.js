...
exports.registerItemProRoutes = registerItemProRoutes;
const parsers_1 = require("../parsers");
const sqlFields_1 = require("../sqlFields");
const pro1stSql_1 = require("../pro1stSql");
function registerItemProRoutes({ app, pool, itemDateField, prefixedDateField, }) {
    const pro1stItemSql = (0, pro1stSql_1.buildQualifiedPro1stSql)();
    const aliasedPro1stItemSql = (0, pro1stSql_1.buildQualifiedPro1stSql)("i.");
    const aliasedExcludedPro1stSql = (0, pro1stSql_1.buildPro1stExcludedSql)("i.");
    const requestDateFields = (req) => ({
        itemDateField: (0, sqlFields_1.dateFieldForBasis)(req.query.date_basis),
        prefixedDateField: (tableAlias) => (0, sqlFields_1.prefixedDateFieldForBasis)(req.query.date_basis, tableAlias),
    });
    // Best sellers (items)
  ...
  `;
    ...
    app.get("/api/items/category-top-items", async (req, res) => {
        ...
        const end = (0, parsers_1.parseDateParam)(req.query.end, "2100-01-01");
        const limit = Math.min(Number(req.query.limit || 10), 50);
        const category = (0, parsers_1.parseTextParam)(req.query.category);
        const locationQ = (0, parsers_1.parseTextParam)(req.query.location);
        const salespersonQ = (0, parsers_1.parseTextParam)(req.query.salesperson);
        if (!category) {
  ...
  `;
    ...
    app.get("/api/items/manufacturer-top-items", async (req, res) => {
        ...
        const end = (0, parsers_1.parseDateParam)(req.query.end, "2100-01-01");
        const limit = Math.min(Number(req.query.limit || 10), 50);
        const manufacturer = (0, parsers_1.parseTextParam)(req.query.manufacturer);
        const locationQ = (0, parsers_1.parseTextParam)(req.query.location);
        const salespersonQ = (0, parsers_1.parseTextParam)(req.query.salesperson);
        if (!manufacturer) {
  ...
  `;
    ...
    WHERE ${prefixedDateField("s")} >= $1
      ...
      AND NOT (${(0, pro1stSql_1.buildQualifiedPro1stSql)("i.")})
    GROUP BY day
    ORDER BY day;
  `;
...
//# sourceMappingURL=itemProRoutes.js.map