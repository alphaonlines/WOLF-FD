...
exports.registerObjectionVotesRoutes = registerObjectionVotesRoutes;
function registerObjectionVotesRoutes(app, pool) {
    // Ensure table exists
    pool.query(`
  ...
  `).catch(() => { });
    ...
    app.get("/api/objection-votes", async (req, res) => {
        ...
        const countsResult = await pool.query(`SELECT objection_id, rebuttal_index, COUNT(*)::int AS count
       FROM objection_votes
       GROUP BY objection_id, rebuttal_index`).catch(() => ({ rows: [] }));
        const userVotesResult = userId
            ? await pool.query(`SELECT objection_id, rebuttal_index FROM objection_votes WHERE user_id = $1`, [userId]).catch(() => ({ rows: [] }))
            : { rows: [] };
        // votes: { [objectionId]: { [rebuttalIndex]: count } }
        const votes = {};
        for (const row of countsResult.rows) {
            if (!votes[row.objection_id])
                votes[row.objection_id] = {};
    ...
    app.post("/api/objection-votes", async (req, res) => {
        ...
        await pool.query(`INSERT INTO objection_votes (user_id, objection_id, rebuttal_index, created_at)
       VALUES ($1, $2, $3, now())
       ON CONFLICT (user_id, objection_id)
       DO UPDATE SET rebuttal_index = EXCLUDED.rebuttal_index, created_at = now()`, [userId, objectionId, rebuttalIndex]);
    ...
    app.delete("/api/objection-votes/:objectionId", async (req, res) => {
        ...
        await pool.query(`DELETE FROM objection_votes WHERE user_id = $1 AND objection_id = $2`, [userId, objectionId]);
        return res.json({ ok: true });
    });
}
//# sourceMappingURL=objectionVotesRoutes.js.map