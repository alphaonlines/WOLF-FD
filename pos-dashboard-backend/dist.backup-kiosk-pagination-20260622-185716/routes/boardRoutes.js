...
function canManageAllMessages(user) {
...
function canUseChannel(user, channel) {
    if (!PRIVATE_CHANNELS.has(channel))
        return true;
    return hasAnyRole(user, ["Owner", "Manager"]);
}
function toNumericUserId(user) {
    if (!user)
        return null;
...
function boardUploadMulter(dir) {
...
function mapUploadRow(row, publicBaseUrl) {
    if (!row || row.upload_id == null)
        return null;
...
function mapMessageRow(row, publicBaseUrl) {
    if (!row)
        return null;
    return {
        id: String(row.id ?? ""),
...
function ensureMappedMessageRow(row, publicBaseUrl) {
...
async function resolveRecipient(pool, recipientUserId) {
...
async function loadMessageForAccess(pool, messageId) {
    const result = await pool.query(`
      SELECT
        m.id,
        m.scope,
...
function canAccessMessage(user, row) {
    if (!user || !row)
        return false;
    if (row.deleted_at)
        return false;
...
function registerBoardRoutes(app, pool, boardUploadsDir, publicBaseUrl) {
    ...
    app.get("/api/board/uploads/:uploadId/:name", async (req, res) => {
      ...
      `, [uploadId]);
        ...
        const row = result.rows[0];
        const filePath = path_1.default.join(boardUploadsDir, String(row.storage_name || ""));
    ...
    app.get("/api/board/messages", async (req, res) => {
        ...
        if (scope === "dm") {
            const otherUserId = (0, parsers_1.parseTaskIdParam)(req.query?.userId);
            if (!otherUserId)
                return res.status(400).json({ error: "invalid userId" });
            if (userId === null)
                return res.status(400).json({ error: "invalid current user" });
            const result = await pool.query(`
        ${buildMessageSelectSql(`WHERE m.scope = 'dm'
        ...
        const result = await pool.query(`
      ${buildMessageSelectSql(`WHERE m.scope = 'channel'
           AND m.deleted_at IS NULL
           AND m.channel = $1
         ORDER BY m.created_at ASC, m.id ASC
         LIMIT 500`)}
    ...
    app.post("/api/board/messages", async (req, res) => {
        ...
        let recipientEmail = "";
        if (scope === "dm") {
            recipientUserId = (0, parsers_1.parseTaskIdParam)(req.body?.recipientUserId);
            if (!recipientUserId)
    ...
    app.patch("/api/board/messages/:messageId", async (req, res) => {
        ...
        if (!user)
            return res.status(401).json({ error: "unauthorized" });
        const messageId = (0, parsers_1.parseTaskIdParam)(req.params.messageId);
        if (!messageId)
            return res.status(400).json({ error: "invalid messageId" });
        const existing = await loadMessageForAccess(pool, messageId);
        if (!existing || !canAccessMessage(user, existing))
            return res.status(404).json({ error: "message not found" });
        if (!canMutateMessage(user, existing))
            return res.status(403).json({ error: "forbidden" });
