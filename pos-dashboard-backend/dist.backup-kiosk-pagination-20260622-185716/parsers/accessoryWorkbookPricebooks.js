...
exports.parseGuardsmanWorkbook = parseGuardsmanWorkbook;
exports.parseGbsProtectallWorkbook = parseGbsProtectallWorkbook;
exports.parseInnovationsWorkbook = parseInnovationsWorkbook;
exports.parseAccessoryReferenceNotes = parseAccessoryReferenceNotes;
...
function detectInnovationCategory(description) {
    const value = description.toLowerCase();
    if (value.includes("bunk"))
        return "Bunk Beds";
    if (value.includes("day bed") || value.includes("daybed"))
        return "Daybeds";
    if (value.includes("ladder"))
        return "Ladders";
    if (value.includes("stair"))
        return "Stair Parts";
    if (value.includes("drawer"))
        return "Storage";
    if (value.includes("slat"))
        return "Slat Kits";
    if (value.includes("rail"))
        return "Rails";
    if (value.includes("mirror"))
        return "Mirrors";
    if (value.includes("frame"))
        return "Frames";
    return "Bedroom Components";
}
function buildCollectionNameFromDescription(description) {
    const token = cleanText(description).split(/\s+/)[0] || "";
    if (!token || token.startsWith("#"))
        return "";
    if (/^(full|extra|metal|ubc|staircase|full)$/i.test(token))
        return "";
    return toTitleCase(token);
}
function readSheetRows(workbookPath, sheetIndex = 0) {
...
async function parseGuardsmanWorkbook(absolutePath) {
...
async function parseGbsProtectallWorkbook(absolutePath) {
    const rows = readSheetRows(absolutePath);
    const parsedRows = [];
    let headerFound = false;
    ...
    for (const rawRow of rows) {
        ...
        if (!sku || !description || unitPrice === null)
            continue;
...
async function parseInnovationsWorkbook(absolutePath) {
    const rows = readSheetRows(absolutePath);
    const parsedRows = [];
    let headerFound = false;
    let sortOrder = 0;
    ...
    if (!parsedRows.length) {
        throw new Error("Innovations preview expects a SKU list workbook such as 'INNOVATION SKU LIST.xlsx' or 'INNOVATIONS PRICING BY SKU.xlsx'.");
    }
    return parsedRows;
}
function parseAccessoryReferenceNotes() {
    return [];
}
//# sourceMappingURL=accessoryWorkbookPricebooks.js.map