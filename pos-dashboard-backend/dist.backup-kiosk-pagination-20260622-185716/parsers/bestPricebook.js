...
exports.parseBestPricebookWorkbook = parseBestPricebookWorkbook;
exports.parseBestReferenceNotes = parseBestReferenceNotes;
exports.isBestResidentialWorkbook = isBestResidentialWorkbook;
exports.chooseBestPreferredHoldingUpload = chooseBestPreferredHoldingUpload;
...
function detectProductType(text, category) {
    ...
    if (value.includes("sectional"))
        return "sectional";
    if (value.includes("chair & a half"))
        return "chair and a half";
    if (value.includes("chair"))
        return "chair";
    if (value.includes("ottoman"))
        return "ottoman";
    if (value.includes("barstool"))
        return "barstool";
    if (value.includes("dining chair"))
        return "dining chair";
...
function detectFeatureTags(text, headers) {
...
function scoreUploadName(fileName) {
    const value = fileName.toLowerCase();
    let score = 0;
    if (value.endsWith(".xlsx") || value.endsWith(".xls"))
        score += 200;
    if (value.includes("residential price list"))
        score += 500;
    if (value.includes("compressed"))
        score += 25;
    if (value.includes("grade change"))
        score -= 100;
    ...
    if (value.includes("warranty"))
        score -= 70;
    if (value.includes("cheat"))
        score -= 60;
    return score;
}
function choosePreferredPricebookFile(filePaths) {
...
function readSheetRows(workbookPath, sheetName) {
    const workbook = xlsx_1.default.readFile(workbookPath, { dense: true });
    const sheet = workbook.Sheets[sheetName];
    if (!sheet)
        return [];
    return xlsx_1.default.utils.sheet_to_json(sheet, { header: 1, blankrows: false, defval: "" });
}
function buildHeaderLabels(previousRows, headerRow) {
    const labels = new Map();
    const previousRowText = previousRows
        .map((row) => row.map((value) => cleanText(value)).filter(Boolean).join(" "))
        .filter(Boolean)
        .reverse();
    const globalDescriptor = previousRowText.find((value) => /special cording/i.test(value)) ||
...
function chooseBasePrice(pricing) {
    const preferred = pricing.find((entry) => entry.label === "Fabric A") ||
...
function isLikelyCollectionName(value, nonEmptyValuesLength) {
    const normalized = cleanText(value);
    if (!normalized)
        return false;
    if (nonEmptyValuesLength > 4)
        return false;
    if (/collection|how to order|matching products|note:/i.test(normalized))
        return false;
...
function detectDimensionColumns(row) {
    ...
    if (heightIndex < 0 || widthIndex < 0 || depthIndex < 0)
        return null;
    return {
...
function matchesDimensionLabel(row, label) {
    const normalizedLabel = normalizeDimensionLabel(label);
    if (!normalizedLabel || normalizedLabel === "seat")
        return false;
    const description = normalizeDimensionLabel(row.description);
    const productType = normalizeDimensionLabel(row.productType);
    if (normalizedLabel.includes("storage ottoman"))
        return description.includes("storage ottoman");
    if (normalizedLabel.includes("club chair"))
        return description.includes("club chair");
    ...
    if (normalizedLabel.includes("loveseat"))
        return description.includes("loveseat") || productType.includes("loveseat");
    if (normalizedLabel.includes("sofa"))
        return description.includes("sofa") || productType === "sofa";
    if (normalizedLabel.includes("recliner"))
        return productType.includes("recliner") || description.includes("recliner");
    if (normalizedLabel.includes("ottoman"))
        return description.includes("ottoman") || productType === "ottoman";
    if (normalizedLabel.includes("barstool"))
        return description.includes("barstool") || productType === "barstool";
    if (normalizedLabel.includes("bench"))
        return description.includes("bench") || productType === "bench";
    if (normalizedLabel.includes("chair"))
        return description.includes("chair") || productType.includes("chair");
    return description.includes(normalizedLabel) || productType.includes(normalizedLabel);
}
function applyDimensionsRow(row, columns, currentSectionRows) {
    ...
    const parts = [];
    if (heightText)
        parts.push(`H ${heightText}`);
    if (widthText)
        parts.push(`W ${widthText}`);
    if (depthText)
        parts.push(`D ${depthText}`);
...
function buildSourceNote(params) {
    ...
    if (params.collectionName)
        parts.push(`Collection ${params.collectionName}`);
    if (params.category)
        parts.push(params.category);
    if (params.pricing.length) {
        parts.push(`Pricing: ${params.pricing
            .slice(0, 8)
...
function parseProductSheet(sheetName, workbookPath, sortOrderStart, modelCollectionMap) {
    ...
    let recentHeaderRows = [];
    let currentHeaderLabels = new Map();
    let pendingNotes = [];
    let matchingProducts = [];
    let currentSectionRows = [];
    let currentDimensionColumns = null;
    ...
    for (let index = 0; index < rows.length; index += 1) {
        ...
        const looksLikeHeaderRow = hasFabricHeader ||
        ...
        if (/^[A-Z0-9]/.test(first) && !SKIPPED_PRODUCT_CODES.has(first.toUpperCase()) && hasPriceNumbers) {
            const pricing = Array.from(currentHeaderLabels.entries())
                ...
                .filter((entry) => Boolean(entry.label) && entry.price !== null);
            if (!pricing.length)
                continue;
        ...
        if (row.some((value) => /matching product/i.test(value))) {
            ...
            if (related.length)
                matchingProducts.push(...related);
        }
    }
    return parsedRows;
}
function parseGradeChangeSheet(workbookPath) {
    ...
    for (const row of rows) {
        ...
        if (!series || series === "SERIES")
            continue;
        if (!oldGrade || oldGrade === "GRADE" || !newGrade || newGrade === "GRADE")
            continue;
        mappings.push(`${series}: ${oldGrade} -> ${newGrade}`);
    }
    return mappings;
}
function buildBundleReferenceNote(bundleDir) {
...
function buildModelCollectionMap(workbookPath) {
    ...
    for (const row of rows) {
        ...
        for (const [rawName, rawModels] of pairs) {
            if (!rawName || !rawModels || rawName === "NAME" || rawModels === "MODEL")
                continue;
            ...
            for (const token of tokens) {
                ...
                if (family)
                    modelMap.set(family.toUpperCase(), { collectionName, collectionCode });
            }
        }
    }
    return modelMap;
...
function resolveCollectionMeta(params) {
    ...
    const found = params.modelCollectionMap.get(skuUpper) ||
...
function normalizeCoverLabel(label) {
    const value = cleanText(label);
    if (!value)
        return "";
    if (/^special cording(?:\s+\([^)]+\))?\s+\d+[a-z0-9]*$/i.test(value))
        return "Special Cording";
    if (/^leather(?:\s+\([^)]+\))?\s+\d+[a-z0-9]*$/i.test(value))
        return "Leather";
    if (/^diamond pricing(?:\s+\([^)]+\))?\s+\d+[a-z0-9]*$/i.test(value))
        return "Diamond Pricing";
    if (/^natural instincts(?:\s+\([^)]+\))?\s+\d+[a-z0-9]*$/i.test(value))
        return "Natural Instincts";
    if (/^[a-g]\s+\d+[a-z0-9]*$/i.test(value))
        return `Fabric ${value.slice(0, 1).toUpperCase()}`;
    if (/^fabric\s+[a-g]$/i.test(value)) {
        return `Fabric ${value.slice(-1).toUpperCase()}`;
    }
    if (/^special cording\s+\d+[a-z]*$/i.test(value))
        return "Special Cording";
    if (/^leather\s+\d+[a-z]*$/i.test(value))
        return "Leather";
    if (/^diamond pricing\s+\d+[a-z]*$/i.test(value))
        return "Diamond Pricing";
    if (/^natural instincts\s+\d+[a-z]*$/i.test(value))
        return "Natural Instincts";
    if (/^\d+\s+\d+$/.test(value))
        return "Leather Matrix";
    return value;
}
async function parseBestPricebookWorkbook(absolutePath) {
    const allRows = [];
...
async function parseBestReferenceNotes(absolutePath) {
    const notes = [];
    const bundleDir = path_1.default.dirname(absolutePath);
    const workbook = xlsx_1.default.readFile(absolutePath, { dense: true });
    const warrantyRows = xlsx_1.default.utils.sheet_to_json(workbook.Sheets["Warranty"], {
        header: 1,
        blankrows: false,
        defval: "",
    });
    ...
    const gradeChangePath = fs_1.default
        .readdirSync(bundleDir)
        .find((entry) => /grade change cheat sheet\.xls$/i.test(entry));
    if (gradeChangePath) {
        const mappings = parseGradeChangeSheet(path_1.default.join(bundleDir, gradeChangePath));
        if (mappings.length) {
            notes.push({
                manufacturer: "Best",
                manufacturerSlug: "best",
    ...
    const supportDocs = fs_1.default
        .readdirSync(bundleDir)
...
function isBestResidentialWorkbook(filePath) {
    const lower = filePath.toLowerCase();
    return ((lower.endsWith(".xls") || lower.endsWith(".xlsx")) &&
        lower.includes("best") &&
        lower.includes("residential price list"));
}
function chooseBestPreferredHoldingUpload(filePaths) {
    return choosePreferredPricebookFile(filePaths);
}
//# sourceMappingURL=bestPricebook.js.map