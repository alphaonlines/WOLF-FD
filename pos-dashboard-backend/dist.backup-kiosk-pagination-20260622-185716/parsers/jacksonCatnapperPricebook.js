...
exports.parseJacksonCatnapperPricebookPdf = parseJacksonCatnapperPricebookPdf;
exports.parseJacksonCatnapperPricebookText = parseJacksonCatnapperPricebookText;
exports.parseJacksonCatnapperReferenceNotes = parseJacksonCatnapperReferenceNotes;
...
function extractPdfText(filePath) {
    return new Promise((resolve, reject) => {
        const child = (0, child_process_1.spawn)("pdftotext", ["-layout", filePath, "-"]);
        let out = "";
        let err = "";
        ...
        child.on("close", (code) => {
            if (code === 0)
                ...
                reject(new Error(clean(err) || `pdftotext exited with code ${code ?? "unknown"}`));
        });
    });
}
// ─── brand detection ─────────────────────────────────────────────────────────
function detectBrand(text) {
    ...
    if (upper.includes("CATNAPPER FURNITURE KIOSK"))
        return "Catnapper";
    if (upper.includes("JACKSON FURNITURE KIOSK"))
        return "Jackson";
...
function sectionToCategory(section) {
    ...
    if (s.includes("reclining sofas") || s.includes("motion"))
        return "Motion Upholstery";
    if (s.includes("reclining chair") || s.includes("lift chair") || s.includes("pow-r-lift"))
        return "Reclining Chairs";
    if (s.includes("stationary"))
        return "Stationary Upholstery";
    if (s.includes("sleeper"))
        return "Sleepers";
    if (s.includes("accent"))
        return "Accents";
...
function extractFeatureTags(description, styleNote, section) {
...
function detectProductType(description) {
    ...
    if (/comfort\s+sack/.test(d))
        return "comfort sack";
    if (/footstool/.test(d))
        return "footstool";
    if (/castered\s+cocktail\s+ottoman|cocktail\s+ottoman/.test(d))
        return "cocktail ottoman";
    if (/\botto(?:man)?\b/.test(d))
        return "ottoman";
    if (/modular\s+sectional|sectional/.test(d))
        return "sectional";
    if (/pow-?r[- ]lift|lift\s+chair/.test(d))
        return "lift chair";
    if (/rocker\s+recliner|glider\s+recliner|wall\s+hugger\s+recliner|lay\s+flat\s+recliner|power\s+lay\s+flat\s+recliner|power\s+recliner|\brecliner\b/.test(d))
        return "recliner";
    if (/reclining\s+sofa|sofa\s+w\/?\d*\s+recliners?/.test(d))
        return "reclining sofa";
    if (/reclining\s+loveseat/.test(d))
        return "reclining loveseat";
    if (/sleeper/.test(d))
        return "sleeper sofa";
    if (/loveseat/.test(d))
        return "loveseat";
    if (/chair\s+1\s*\/\s*2|chair\s+1\.5/.test(d))
        return "chair and a half";
    if (/accent\s+chair/.test(d))
        return "accent chair";
    if (/\bchair\b/.test(d))
        return "chair";
    if (/\bsofa\b/.test(d))
    ...
    if (/corner/.test(d))
        return "corner piece";
    if (/armless/.test(d))
        return "armless piece";
    if (/section/.test(d))
        return "sectional piece";
    return "furniture";
}
function detectMaterial(section, fabricNumber) {
    const s = `${section} ${fabricNumber}`.toLowerCase();
    if (s.includes("leather") || s.includes("italian"))
        return "leather";
    return "fabric";
}
// ─── line classification ──────────────────────────────────────────────────────
...
const ITEM_ROW_RE = /^\s{5,}(.+?)\s{2,}(\S+)\s{2,}(\S+)\s{2,}(.*?)\s+(\d+)\s+(\d+)\s+(\d+)\s+(\d+)\s+([\d.]+)\s+([\d.]+)\s+\$\s*([\d,]+\.\d{2})\s*$/;
...
const SUBTYPE_ITEM_ROW_RE = /^(Modular\s+Sectional|Top\s+Grain\s+Leather\s+Touch|Sectional)\s+(.+?)\s{2,}(\S+)\s{2,}(\S+)\s{2,}(.*?)\s+(\d+)\s+(\d+)\s+(\d+)\s+(\d+)\s+([\d.]+)\s+([\d.]+)\s+\$\s*([\d,]+\.\d{2})\s*$/;
...
const STYLE_INLINE_ITEM_ROW_RE = /^(\d{2,5}[a-z]?)\s+(\S+)\s+(.+?)\s{2,}(\S+)\s{2,}(\S+)\s{2,}(.*?)\s+(\d+)\s+(\d+)\s+(\d+)\s+(\d+)\s+([\d.]+)\s+([\d.]+)\s+\$\s*([\d,]+\.\d{2})\s*$/;
// Style line: number at left margin followed by a name and optional feature notes
const STYLE_LINE_RE = /^\s{0,10}(\d{2,5}[a-z]?)\s+([A-Za-z].+?)\s*$/i;
function cleanStyleName(value) {
    return clean(value)
        .replace(/\s*\(continued\)\s*/i, "")
        .replace(/\s*(?:-\s*)?(?:POWER\s+UPGRADE\s+AVAILABLE|TOP\s+GRAIN\s+LEATHER\s+TOUCH|CAMOUFLAGE|AVAILABLE\s+JACKSON\s+MS\s+ONLY).*$/i, "")
        .trim();
}
...
function shouldSkip(line) {
    ...
    if (!c)
        return true;
    if (/^(Effective|Updated)\b/i.test(c))
        return true;
    if (/^(JACKSON|CATNAPPER)\s+FURNITURE\s+KIOSK/i.test(c))
        return true;
    if (/^Ship\s+Point:/i.test(c))
        return true;
    if (/^STYLE\s+DESCRIPTION\s+SKU\b/i.test(c))
        return true;
    if (/^KIOSK\s*$/i.test(c) || /^PRICE\s*$/i.test(c))
        return true;
    if (/^\*If ordering a truckload/i.test(c))
        return true;
    if (/^Page\s+\d+\b/i.test(c))
        return true;
    if (/Pillow\s+Fabric\b/i.test(c))
        return true;
    if (/^Ship\s+Point\s+varies/i.test(c))
        return true;
    return false;
}
// ─── main parser ─────────────────────────────────────────────────────────────
async function parseJacksonCatnapperPricebookPdf(filePath, execFileAsync) {
    void execFileAsync;
    const text = await extractPdfText(filePath);
    return parseJacksonCatnapperPricebookText(text);
}
function parseJacksonCatnapperPricebookText(text) {
    if (!text.trim())
    ...
    for (const rawLine of text.split("\n")) {
        ...
        // Style line (group header). Check this before standalone feature notes so
        // lines like "108 Hollifield  POWER UPGRADE AVAILABLE" do not inherit the
        // previous style collection.
        const styleMatch = line.match(STYLE_LINE_RE);
        if (styleMatch && !ITEM_ROW_RE.test(line)) {
            currentStyleCode = clean(styleMatch[1]);
            currentStyleName = cleanStyleName(styleMatch[2]);
            currentStyleNote = POWER_UPGRADE_RE.test(compact) ? "power-upgrade-available" : "";
            currentSubtype = "";
            continue;
        }
        ...
        const dimensionsText = widthInches && heightInches && depthInches
...
function parseJacksonCatnapperReferenceNotes() {
...
//# sourceMappingURL=jacksonCatnapperPricebook.js.map