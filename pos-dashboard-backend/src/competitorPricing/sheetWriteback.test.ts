...
describe('sheetWriteback', () => {
  ...
  it('plans writes to both comp price columns and skips low-confidence rows in dry-run mode', async () => {
    const { fetchImpl, calls } = makeFetch();
    const lowOnly = { ...resultRow, sourceRow: 3, ashley: undefined, furniture4Less: { ...resultRow.furniture4Less!, confidence: 'low' as const } };
    ...
    expect(summary.updatedCells).toBe(2);
    expect(summary.columns).toEqual({ ahsCompColumn: 'R', fflCompColumn: 'S' });
  ...
  it('writes values, colors comp price cells, and adds notes through Sheets batch APIs', async () => {
    ...
    expect(formatCall?.body.requests.length).toBe(4);
