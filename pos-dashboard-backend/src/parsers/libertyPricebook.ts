...
const ITEM_LINE_PATTERN =
  /^(S\s+)?([A-Z0-9][A-Z0-9-]+)\s+(.+?)\s{2,}(W\d+(?:\.\d+)?\s+x\s+D\d+(?:\.\d+)?\s+x\s+H\d+(?:\.\d+)?)\s+([\d.]+)\s+([\d.]+)\s+\$([\d,]+\.\d{2})\s*$/i;
const SET_LINE_PATTERN =
  /^(?:Opt\s+)?(.+?)\s+\(([A-Z0-9-]+)\)\s+([\d.]+)\s+([\d.]+)\s+\$([\d,]+\.\d{2})\s*$/i;
...
function parseDimensions(dimensionsText: string) {
...
function detectColorFinish(rawDescription: string, swatchName: string) {
  const explicit = rawDescription.match(/-\s*([A-Za-z][A-Za-z /&-]+)$/);
  if (explicit) return explicit[1].trim();
  if (/\bswatch\b/i.test(rawDescription)) return rawDescription.replace(/\s+swatch$/i, "").trim();
  return swatchName.trim();
}

function detectColorFamily(input: string) {
  ...
  for (const family of families) {
    if (value.includes(family)) return family === "grey" ? "gray" : family;
  }
  return "";
}
...
function detectMaterial(text: string) {
  ...
  if (value.includes("fabric") || value.includes("swatch") || value.includes("uph")) return "fabric";
  if (
    value.includes("wood") ||
    value.includes("oak") ||
...
function detectShape(text: string) {
  const value = text.toLowerCase();
  if (value.includes("round")) return "round";
  if (value.includes("oval")) return "oval";
  if (value.includes("square")) return "square";
  if (value.includes("rectangle") || value.includes("rectangular")) return "rectangular";
  return "";
}

function detectProductType(text: string, isSet: boolean, isSwatch: boolean) {
  const value = text.toLowerCase();
  ...
  if (value.includes("cocktail table")) return "cocktail table";
  if (value.includes("end table")) return "end table";
  if (value.includes("chair side") || value.includes("chairside")) return "chairside table";
  if (value.includes("sofa table")) return "sofa table";
  if (value.includes("console")) return "console";
  if (value.includes("cabinet")) return "cabinet";
  if (value.includes("bookcase")) return "bookcase";
  if (value.includes("bed")) return "bed";
  if (value.includes("dresser")) return "dresser";
  if (value.includes("chest")) return "chest";
  if (value.includes("night stand") || value.includes("nightstand")) return "nightstand";
  if (value.includes("mirror")) return "mirror";
  if (value.includes("bench")) return "bench";
  if (value.includes("server")) return "server";
  if (value.includes("buffet")) return "buffet";
...
function detectHardwareOptions(text: string) {
  const value = text.toLowerCase();
  const matches: string[] = [];
  if (value.includes("custom hardware")) matches.push("custom hardware");
  if (value.includes("bronze hardware")) matches.push("bronze hardware");
  if (value.includes("nickel hardware")) matches.push("nickel hardware");
  return matches;
}

function detectCushionOptions(text: string) {
  const value = text.toLowerCase();
  const matches: string[] = [];
  if (value.includes("reversible cushion")) matches.push("reversible cushion");
  if (value.includes("attached cushion")) matches.push("attached cushion");
  if (value.includes("loose cushion")) matches.push("loose cushion");
  if (value.includes("memory foam")) matches.push("memory foam");
  return matches;
}

function detectFeatureTags(text: string, dimensionsText: string) {
...
function tokenizeSearchKeywords(values: Array<string | number | null | undefined>) {
...
function buildSourceNote(parts: string[]) {
  return parts.filter(Boolean).join(" | ");
}

export async function parseLibertyPricebookPdf(
  absolutePath: string,
  ...
  let currentCategory = "";
  let currentCollection: CollectionContext | null = null;
  let sortOrder = 0;
  const rows = new Map<string, ParsedManufacturerCatalogRow>();

  for (const rawLine of lines) {
    const normalized = normalizeLine(rawLine);
    const trimmed = normalized.trim();
    if (!trimmed) continue;
    if (trimmed.startsWith("*S=") || trimmed === "www.mylibertyfurniture.com") continue;
    if (trimmed === "Sku Number" || trimmed.startsWith("Sku Number ") || trimmed.startsWith("Description")) continue;
    if (trimmed === "Dimensions" || trimmed === "Cubes" || trimmed === "Weight" || trimmed === "Price") continue;
    ...
    if (itemMatch && currentCollection) {
      ...
      const featureTags = detectFeatureTags(
        `${rawDescription} ${currentCollection.isNewProduct ? "new product" : ""} ${isSample ? "sample" : ""}`,
        dimensionsText
      );
...
export async function parseLibertyReferenceNotesFromPdf(
  absolutePath: string,
  execFileAsync: ExecFileAsyncLike
): Promise<ParsedManufacturerReferenceNote[]> {
  const result = await execFileAsync("pdftotext", [absolutePath, "-"], { timeout: 120000 });
  const text = String(result.stdout ?? "");

  const notes: ParsedManufacturerReferenceNote[] = [];
  const pushNote = (noteType: string, title: string, content: string) => {
  ...
  if (returnPolicyMatch) {
    ...
    const warrantyMatch = returnPolicyMatch[1].match(
      /Manufacturers warranty is one year from the date of invoice[\s\S]*?(?=\d\)|$)/i
    );
    if (warrantyMatch) {
