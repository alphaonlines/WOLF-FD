...
function quoteDatePlusDays(days: number): string {
  const base = new Date();
...
function withLimit(sql: string, limit = 20): string {
  const trimmed = sql.trim().replace(/;$/, "");
  return `${trimmed} LIMIT ${limit}`;
}

function mapSalesOrderMatch(row: any) {
  return {
...
function mapUpsVisitMatch(row: any) {
  return {
    id: String(row.id ?? ""),
...
export function registerCrmQuoteRoutes(app: Express, pool: Pool) {
  const handleCreateQuote = async (req: any, res: any) => {
    ...
    try {
      ...
      const salesOrders = salesOrderRows.rows.map((row: any) => ({
        ...mapSalesOrderMatch(row),
        match_confidence: "high",
      }));

      const upsWhere: string[] = [];
      const upsParams: string[] = [];
      if (normalizedPhone) {
      ...
      let upsVisits = upsVisitRows.rows.map((row: any) => ({
