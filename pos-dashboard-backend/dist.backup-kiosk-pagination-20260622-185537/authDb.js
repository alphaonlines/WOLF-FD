...
exports.createAuthDbHelpers = createAuthDbHelpers;
function createAuthDbHelpers({ pool, authCookieName, parseCookies, sha256Hex, buildAuthUser, normalizeRoleList, }) {
    const findAuthUserBySessionToken = async (token) => {
        if (!token)
            return null;
    ...
    const currentAuthUserFromReq = async (req) => {
    ...
    const getRoleIdMap = async () => {
        ...
        for (const row of r.rows) {
            ...
            if (k && Number.isFinite(id))
                out[k] = id;
        }
        return out;
    };
    const setUserRolesByKeys = async (userId, roleKeys) => {
        ...
        for (const roleId of roleIds) {
            await pool.query(`INSERT INTO user_roles (user_id, role_id, created_at) VALUES ($1, $2, now()) ON CONFLICT DO NOTHING`, [userId, roleId]);
        }
    };
