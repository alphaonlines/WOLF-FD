...
exports.getStoreWeatherSnapshot = getStoreWeatherSnapshot;
const runtimeConfig_1 = require("./runtimeConfig");
const WEATHER_CACHE_TTL_MS = Math.max(Number((0, runtimeConfig_1.envString)("CRM_WEATHER_CACHE_TTL_MS", "900000")) || 900000, 60000);
const WEATHER_TIMEOUT_MS = Math.max(Number((0, runtimeConfig_1.envString)("CRM_WEATHER_TIMEOUT_MS", "3500")) || 3500, 1000);
const WEATHER_USER_AGENT = (0, runtimeConfig_1.envString)("CRM_WEATHER_USER_AGENT", "WOLF-FD CRM Weather (ops@wolf.discount)") ||
...
let parsedStoreOverrides = null;
function parseStoreOverrides() {
    if (parsedStoreOverrides)
        return parsedStoreOverrides;
    const raw = (0, runtimeConfig_1.envString)("CRM_WEATHER_STORE_COORDS_JSON", "");
    if (!raw) {
    ...
    try {
        ...
        for (const [store, value] of Object.entries(parsed)) {
            ...
            const label = String(value.label || store).trim();
...
async function getStoreWeatherSnapshot(store) {
    ...
    try {
        const pointData = await fetchJson(`https://api.weather.gov/points/${location.latitude.toFixed(4)},${location.longitude.toFixed(4)}`);
        const forecastUrl = pointData?.properties?.forecastHourly || pointData?.properties?.forecast || pointData?.properties?.forecastGridData;
        if (!forecastUrl) {
...
//# sourceMappingURL=crmWeather.js.map