...
exports.attachMeetingSignaling = attachMeetingSignaling;
const socket_io_1 = require("socket.io");
const MEETING_ROOM_ID = "team-meeting";
function attachMeetingSignaling(httpServer) {
    ...
    io.on("connection", (socket) => {
        ...
        socket.on("join-meeting", ({ userName, userId }) => {
            socket.join(MEETING_ROOM_ID);
            if (!rooms.has(MEETING_ROOM_ID))
                rooms.set(MEETING_ROOM_ID, new Map());
        ...
        socket.on("disconnect", () => {
            ...
            if (room) {
                ...
                if (p)
                    console.log(`[Meeting] ${p.userName} left`);
                room.delete(socket.id);
                if (room.size === 0)
                    rooms.delete(MEETING_ROOM_ID);
            }
