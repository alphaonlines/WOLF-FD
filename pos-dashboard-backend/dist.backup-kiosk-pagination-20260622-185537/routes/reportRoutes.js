...
function registerReportRoutes({ app, pool, prefixedDateField }) {
    ...
    app.get("/api/report/sales-summary", async (req, res) => {
        ...
        const categoryQ = (0, parsers_1.parseTextParam)(req.query.category);
        const manufacturerQ = (0, parsers_1.parseTextParam)(req.query.manufacturer);
    ...
    `;
        try {
            ...
            res.json({
                ...
                dimension,
                rows: rowsRes.rows,
                distinctTicketCount: rowsRes.rows[0]?.distinct_ticket_count === null || rowsRes.rows[0]?.distinct_ticket_count === undefined
                    ? 0
                    : Number(rowsRes.rows[0].distinct_ticket_count),
                availableCategories: categoriesRes.rows.map((r) => r.category).filter((v) => v),
                availableManufacturers: manufacturersRes.rows.map((r) => r.manufacturer).filter((v) => v),
            });
        }
        catch (err) {
