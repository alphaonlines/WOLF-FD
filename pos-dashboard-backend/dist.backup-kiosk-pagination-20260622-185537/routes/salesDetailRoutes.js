...
exports.registerSalesDetailRoutes = registerSalesDetailRoutes;
const parsers_1 = require("../parsers");
const sqlFields_1 = require("../sqlFields");
const pro1stSql_1 = require("../pro1stSql");
const itemProRoutes_1 = require("./itemProRoutes");
function registerSalesDetailRoutes({ app, pool, itemDateField, prefixedDateField, }) {
    ...
    app.get("/api/salesperson-tickets", async (req, res) => {
        ...
        const itemDateField = (0, sqlFields_1.dateFieldForBasis)(req.query.date_basis);
  ...
  `;
    ...
    app.get("/api/open-location-tickets", async (req, res) => {
        const storeQ = (0, parsers_1.parseTextParam)(req.query.store);
    ...
    `;
        ...
        const returnedLocations = Array.from(new Set(result.rows.map((row) => String(row.location ?? "").trim()).filter(Boolean))).sort((a, b) => a.localeCompare(b));
        res.json({
  ...
  `;
    ...
    (0, itemProRoutes_1.registerItemProRoutes)({ app, pool, itemDateField, prefixedDateField });
}
//# sourceMappingURL=salesDetailRoutes.js.map