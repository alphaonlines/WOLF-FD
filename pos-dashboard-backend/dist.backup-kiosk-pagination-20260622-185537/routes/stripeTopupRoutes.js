...
exports.BOTBOT_STRIPE_TOKEN_PACKS = void 0;
exports.registerStripeTopupRoutes = registerStripeTopupRoutes;
const express_1 = __importDefault(require("express"));
const stripe_1 = __importDefault(require("stripe"));
const TOKEN_RATE_PER_USD = 10000;
const PACK_DOLLARS = [1, 5, 10, 25, 50, 100, 250];
const packLabel = (dollars) => {
    if (dollars === 1)
        return "$1 Starter";
    if (dollars === 25)
        return "$25 Team Pack";
...
const packDescription = (dollars, tokens) => {
    if (dollars === 1)
        return "Entry refill for quick dashboard questions and one-off AI assists.";
    if (dollars === 25)
        return "Practical team refill for steady sales-floor and back-office BotBot use.";
    if (dollars === 250)
        return "Heavy refill for high-volume WOLFbot operations and manager workflows.";
    return `${tokens.toLocaleString()} BotBot tokens at the standard $1 = 10,000 token rate.`;
};
exports.BOTBOT_STRIPE_TOKEN_PACKS = PACK_DOLLARS.map((dollars) => {
...
const normText = (value) => (typeof value === "string" ? value.trim() : "");
...
const applyPurchasedTokens = async (pool, userId, modelKey, creditsRaw) => {
...
const insertStripeEvent = async (pool, eventId, sessionId, eventType, userId, customerEmail, packId, modelKey, tokens, amountTotal, currency, rawPayload, status) => {
...
function registerStripeTopupRoutes({ app, pool, webhookPath, webhookSecret, secretKey, publicBaseUrl, defaultModelKey, externalLedgerToken = "", }) {
    const stripe = new stripe_1.default(secretKey || "sk_test_placeholder", { apiVersion: stripeApiVersion });
    ...
    const safeReturnUrl = (value, fallback) => {
        ...
        try {
            ...
            if (url.protocol !== "https:" && url.protocol !== "http:")
                return fallback;
            if (!url.hostname.endsWith("wolf.discount"))
                return fallback;
            return url.toString();
        }
        catch {
    ...
    const createCheckoutForUser = async ({ userId, userEmail, pack, successUrl, cancelUrl, }) => {
        if (!secretKey)
    ...
    app.get("/api/botbot/token-packs", async (_req, res) => {
        return res.json({ ok: true, rate: { usd: 1, tokens: TOKEN_RATE_PER_USD }, packs: exports.BOTBOT_STRIPE_TOKEN_PACKS.map(publicPack) });
    });
    app.post("/api/botbot/token-packs/checkout", async (req, res) => {
        ...
        const pack = exports.BOTBOT_STRIPE_TOKEN_PACKS.find((candidate) => candidate.id === packId);
        if (!pack)
    ...
    app.post("/api/botbot/external/token-packs/checkout", async (req, res) => {
        ...
        const userResult = await pool.query(`SELECT id, name, email
       FROM users
       WHERE lower(email) = $1
    ...
    app.post(webhookPath, express_1.default.raw({ type: "application/json" }), async (req, res) => {
        ...
        try {
            const eventRowId = await insertStripeEvent(pool, eventId, sessionId, event.type, userId, customerEmail, pack.id, modelKey, tokens, amountTotal, currency, event, "auto_credited");
            if (!eventRowId) {
...
//# sourceMappingURL=stripeTopupRoutes.js.map