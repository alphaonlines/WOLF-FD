...
exports.registerPublicSocialRoutes = registerPublicSocialRoutes;
exports.registerSocialRoutes = registerSocialRoutes;
const fs_1 = __importDefault(require("fs"));
const path_1 = __importDefault(require("path"));
const multer_1 = __importDefault(require("multer"));
const SOCIAL_PLATFORMS = ["facebook", "instagram", "google"];
...
function normalizeAssetKind(file) {
...
function socialUploadMulter(dir) {
...
function mapAssetRow(row, publicBaseUrl) {
...
function mapPostRow(row, asset, jobs) {
    return {
        id: String(row.id ?? ""),
        ...
        platforms: Array.isArray(row.platforms) ? row.platforms.map((value) => String(value)) : [],
        platformAccountIds: row.platform_account_ids && typeof row.platform_account_ids === "object" ? row.platform_account_ids : {},
        asset,
...
async function loadPosts(pool, publicBaseUrl, filters) {
    const where = [];
    const params = [];
    if (filters?.status) {
  ...
  `;
    ...
    const assetIds = Array.from(new Set(posts.map((row) => Number(row.asset_id)).filter((id) => Number.isFinite(id))));
    const postIds = posts.map((row) => Number(row.id)).filter((id) => Number.isFinite(id));
    ...
    return posts.map((row) => mapPostRow(row, row.asset_id ? assetsById.get(Number(row.asset_id)) || null : null, jobsByPost.get(Number(row.id)) || []));
}
function normalizePlatformAccountIds(raw) {
    const out = {};
...
async function syncJobsForPost(pool, postId, platforms, scheduledForIso, platformAccountIds) {
    await pool.query(`
...
function registerPublicSocialRoutes({ app, pool, socialUploadsDir }) {
    app.get("/api/social/assets/:assetId/:name", async (req, res) => {
      ...
      `, [assetId]);
        ...
        const filePath = path_1.default.join(socialUploadsDir, String(row.storage_name || ""));
...
function registerSocialRoutes({ app, pool, socialUploadsDir, publicBaseUrl, runSocialDueJobsOnce, }) {
    ...
    app.get("/api/social/posts", async (req, res) => {
        ...
        const platform = typeof req.query?.platform === "string" && SOCIAL_PLATFORMS.includes(String(req.query.platform).toLowerCase())
    ...
    app.post("/api/social/posts", async (req, res) => {
        ...
        if (!title && !caption)
            return res.status(400).json({ error: "title or caption is required" });
      ...
      `, [
        ...
        const rows = await loadPosts(pool, publicBaseUrl, {});
        const post = rows.find((item) => Number(item.id) === postId);
    ...
    app.patch("/api/social/posts/:id", async (req, res) => {
        ...
        if (req.body?.caption !== undefined)
            setField("caption", String(req.body.caption || "").trim());
        if (req.body?.status !== undefined)
            setField("status", normalizePostStatus(req.body.status));
        if (req.body?.scheduledFor !== undefined)
            setField("scheduled_for", parseOptionalIso(req.body.scheduledFor));
        if (req.body?.timezone !== undefined)
            setField("timezone", String(req.body.timezone || "America/New_York").trim());
        if (req.body?.linkUrl !== undefined)
            setField("link_url", String(req.body.linkUrl || "").trim());
        if (req.body?.ctaLabel !== undefined)
            setField("cta_label", normalizeGoogleCta(req.body.ctaLabel));
        if (req.body?.googleTopicType !== undefined)
            setField("google_topic_type", normalizeGoogleTopicType(req.body.googleTopicType));
        if (req.body?.googleEventTitle !== undefined)
            setField("google_event_title", String(req.body.googleEventTitle || "").trim());
        if (req.body?.googleEventStart !== undefined)
            setField("google_event_start", parseOptionalIso(req.body.googleEventStart));
        if (req.body?.googleEventEnd !== undefined)
            setField("google_event_end", parseOptionalIso(req.body.googleEventEnd));
        if (req.body?.platforms !== undefined)
            setField("platforms", normalizePlatforms(req.body.platforms));
    ...
    app.post("/api/social/posts/:id/publish-now", async (req, res) => {
      ...
      `, [
            ...
            when,
            resolvedPlatforms,
            JSON.stringify(Object.keys(platformAccountIds).length
                ? platformAccountIds
                : normalizePlatformAccountIds(existing.rows[0]?.platform_account_ids)),
            Number(user.id),
        ]);
        await syncJobsForPost(pool, postId, resolvedPlatforms, when, Object.keys(platformAccountIds).length
            ? platformAccountIds
            : normalizePlatformAccountIds(existing.rows[0]?.platform_account_ids));
