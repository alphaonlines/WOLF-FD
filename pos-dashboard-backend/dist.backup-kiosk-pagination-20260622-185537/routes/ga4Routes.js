...
exports.registerGa4Routes = registerGa4Routes;
const google_auth_library_1 = require("google-auth-library");
const path_1 = __importDefault(require("path"));
const fs_1 = __importDefault(require("fs"));
const GA4_PROPERTY_ID = "257030674";
const GA4_KEY_PATH = path_1.default.resolve("/home/alphahs/secrets/ga4-fd-key.json");
const GA4_TOKEN_PATH = path_1.default.resolve("/home/alphahs/secrets/ga4-oauth-token.json");
...
function getOAuthClient() {
    return new google_auth_library_1.OAuth2Client(CLIENT_ID, CLIENT_SECRET, REDIRECT_URI);
}
async function getBearerToken() {
    if (fs_1.default.existsSync(GA4_TOKEN_PATH)) {
        const stored = JSON.parse(fs_1.default.readFileSync(GA4_TOKEN_PATH, "utf8"));
...
function parseIntMetric(row, index) {
    return parseInt(row?.metricValues?.[index]?.value || "0", 10) || 0;
}
function parseFloatMetric(row, index) {
...
function queryRange(req, prefix = "") {
...
async function fetchRangeStats(token, range) {
...
async function fetchGA4Stats(currentRange, compareRange) {
...
function registerGa4Routes(app) {
    ...
    app.get("/api/ga4-oauth-callback", async (req, res) => {
        ...
        try {
            ...
            fs_1.default.writeFileSync(GA4_TOKEN_PATH, JSON.stringify(tokens, null, 2), { mode: 0o600 });
            statsCache.clear();
            res.send(`
...
//# sourceMappingURL=ga4Routes.js.map