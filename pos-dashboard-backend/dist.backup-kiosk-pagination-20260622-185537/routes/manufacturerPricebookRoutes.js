...
exports.registerManufacturerPricebookRoutes = registerManufacturerPricebookRoutes;
const fs_1 = __importDefault(require("fs"));
const path_1 = __importDefault(require("path"));
const multer_1 = __importDefault(require("multer"));
const XLSX = __importStar(require("xlsx"));
const bestPricebook_1 = require("../parsers/bestPricebook");
const englandPricebook_1 = require("../parsers/englandPricebook");
const libertyPricebook_1 = require("../parsers/libertyPricebook");
const jacksonCatnapperPricebook_1 = require("../parsers/jacksonCatnapperPricebook");
const accessoryWorkbookPricebooks_1 = require("../parsers/accessoryWorkbookPricebooks");
...
const DEFAULT_IMAGE_CANDIDATE_RUNS = [
    {
...
function sanitizeManufacturer(value) {
    return value
        .trim()
        .toLowerCase()
        .replace(/&/g, "and")
...
function inferDocumentType(originalName, explicitType) {
    ...
    if (lower.includes("warranty"))
        return "warranty";
    if (lower.includes("freight"))
        return "freight_policy";
...
function getUploadSelectionScore(uploadRow) {
    let score = 0;
    ...
    if (documentType === "archive")
        score -= 1000;
    if (uploadRow.parent_upload_id)
        score += 25;
    if (/\.xlsx?$/.test(name))
        score += 250;
    if (/\.csv$/.test(name))
        score += 120;
    if (/residential price list/.test(name))
        score += 600;
    if (/price[_ -]?list|pricebook/.test(name))
        score += 180;
    if (/sku list|pricing by sku/.test(name))
        score += 350;
    if (/compressed/.test(name))
        score += 20;
    if (/order form|configurations/.test(name))
        score -= 120;
    if (/kits/.test(name))
        score -= 60;
    if (/warranty/.test(name))
        score -= 100;
    if (/tariff|delivery schedule|schedule|coastal covers|curfab|pattern|cushion/.test(name))
        score -= 220;
    if (/diamond/.test(name))
        score -= 120;
    if (/fabric/.test(name))
        score -= 120;
    if (/grade change|cheat sheet/.test(name))
        score -= 140;
    return score;
}
async function loadUploadChildren(pool, parentUploadId) {
...
function choosePreferredUploadCandidate(rows) {
    return [...rows].sort((left, right) => getUploadSelectionScore(right) - getUploadSelectionScore(left))[0] || null;
}
async function resolveUploadRowForProcessing(pool, uploadRow) {
    const documentType = String(uploadRow.document_type || "pricebook").toLowerCase();
    if (documentType === "archive") {
...
function isSpreadsheetUpload(uploadRow) {
    return SPREADSHEET_FILE_PATTERN.test(`${String(uploadRow.original_name || "")} ${String(uploadRow.storage_name || "")}`.toLowerCase());
}
function getCustomParserAvailable(uploadRow) {
...
function getFieldMatchScore(header, field) {
    ...
    for (const synonym of field.synonyms) {
        ...
        if (normalizedSynonym.includes(normalizedHeader) && normalizedHeader.length >= 3)
...
function suggestMappingsForHeaders(headers) {
...
function inferHeaderRow(rows) {
    ...
    rows.slice(0, 30).forEach((row, rowIndex) => {
        ...
        if (score > best.score)
            best = { rowIndex, score, headers };
    });
    return best;
}
function readSpreadsheetRows(filePath, sheetName) {
...
function analyzeSpreadsheetFile(filePath, uploadRow, savedProfile) {
    const workbook = XLSX.readFile(filePath, { cellDates: false, raw: false });
...
async function loadLatestMappingProfile(pool, manufacturerSlug) {
    const result = await pool.query(`
      SELECT
        id,
        manufacturer,
...
async function saveMappingProfile(input) {
    ...
    `, [
        ...
        path_1.default.extname(String(input.uploadRow.original_name || input.uploadRow.storage_name || "")).replace(".", "").toLowerCase(),
        input.sheetName,
...
function resolveMappedColumnIndex(mapping, headers) {
    if (typeof mapping === "number" && Number.isFinite(mapping))
        return mapping;
...
function buildMappedRows(input) {
    ...
    dataRows.forEach((row, index) => {
        ...
        if (!manufacturer && !category && !sku && !description && basePrice === null)
            return;
        const normalized = {
...
function mapUploadRow(row) {
    return {
        ...
        uploaded_by_user_id: row.uploaded_by_user_id === null || row.uploaded_by_user_id === undefined
            ? null
            : String(row.uploaded_by_user_id),
        parent_upload_id: row.parent_upload_id === null || row.parent_upload_id === undefined
            ? null
    ...
    while (stack.length) {
        ...
        for (const entry of entries) {
            if (entry.name === "__MACOSX" || entry.name.startsWith("."))
                continue;
            const nextPath = path_1.default.join(current, entry.name);
...
async function insertUploadRow(pool, input) {
    const result = await pool.query(`
...
async function extractArchiveChildren(input) {
    const archiveBaseName = path_1.default.parse(String(input.archiveUploadRow.storage_name || "archive")).name;
    const extractFolderName = `${archiveBaseName}__unzipped_${Number(input.archiveUploadRow.id)}`;
    const extractDir = path_1.default.join(input.manufacturerDir, extractFolderName);
    fs_1.default.mkdirSync(extractDir, { recursive: true });
    await input.execFileAsync("unzip", ["-oq", input.archiveAbsolutePath, "-d", extractDir], { timeout: 120000 });
    const extractedFiles = collectFilesRecursively(extractDir).filter((filePath) => ACCEPTED_FILE_PATTERN.test(path_1.default.basename(filePath)));
    const insertedRows = [];
    for (const filePath of extractedFiles) {
        const relativeToManufacturerDir = path_1.default.relative(input.manufacturerDir, filePath).replace(/\\/g, "/");
        const relativePath = path_1.default.join(input.manufacturerSlug, relativeToManufacturerDir).replace(/\\/g, "/");
        ...
        const inserted = await insertUploadRow(input.pool, {
            ...
            storageName: path_1.default.basename(filePath),
            relativePath,
...
function mapCatalogRow(row) {
    return {
        ...
        set_piece_count: row.set_piece_count === null || row.set_piece_count === undefined ? null : Number(row.set_piece_count),
        ...
        hardware_options: Array.isArray(row.hardware_options) ? row.hardware_options.map((value) => String(value)) : [],
        cushion_options: Array.isArray(row.cushion_options) ? row.cushion_options.map((value) => String(value)) : [],
        feature_tags: Array.isArray(row.feature_tags) ? row.feature_tags.map((value) => String(value)) : [],
        search_keywords: Array.isArray(row.search_keywords) ? row.search_keywords.map((value) => String(value)) : [],
        image_urls: Array.isArray(row.image_urls) ? row.image_urls.map((value) => String(value)) : [],
        source_note: String(row.source_note ?? ""),
        source_sort_order: Number(row.source_sort_order ?? 0),
    };
}
function mapKioskProductRow(row) {
    return {
        id: String(row.id ?? ""),
        manufacturer: String(row.manufacturer ?? ""),
        manufacturer_slug: String(row.manufacturer_slug ?? ""),
...
function normalizePublicPathPrefix(prefix) {
    return String(prefix || "").replace(/\/+$/, "");
}
function candidateIdFor(run, productId, reviewImage) {
    const fileName = path_1.default.basename(reviewImage || "candidate.jpg");
    return `${run.manufacturerSlug}:${productId}:${fileName}`;
}
function loadKioskImageCandidates(runs, productIds) {
    const rows = [];
    for (const run of runs) {
        const csvPath = path_1.default.join(run.runDir, "match_candidates.csv");
        if (!fs_1.default.existsSync(csvPath))
            continue;
        const parsed = parseCsvRows(fs_1.default.readFileSync(csvPath, "utf8"));
        ...
        for (const raw of parsed) {
            ...
            if (!productId || !reviewImage)
                continue;
            if (productIds && !productIds.has(productId))
                continue;
            const fileName = path_1.default.basename(reviewImage);
            const localImagePath = path_1.default.join(run.runDir, reviewImage);
            if (!fs_1.default.existsSync(localImagePath))
                continue;
            rows.push({
...
function parseProductIdList(raw) {
...
function hasKioskImageApprovalAccess(req) {
    const user = req?.authUser || {};
    const roles = Array.isArray(user.roles) ? user.roles.map((role) => String(role)) : [];
    const permissions = Array.isArray(user.permissions) ? user.permissions.map((permission) => String(permission)) : [];
    return roles.includes("Owner") || permissions.includes(SHOP_KIOSK_IMAGE_APPROVAL_PERMISSION);
}
function requireKioskImageApproval(req, res, next) {
    if (!hasKioskImageApprovalAccess(req))
        return res.status(403).json({ ok: false, error: "forbidden" });
    return next();
}
async function loadUploadByIdOr404(pool, uploadId, res) {
...
async function parseUploadRows(input) {
    ...
    const filePath = path_1.default.join(input.holdingDir, String(resolvedUploadRow.relative_path || ""));
    ...
    if (manufacturerSlug === "liberty") {
        return (0, libertyPricebook_1.parseLibertyPricebookPdf)(filePath, input.execFileAsync);
    }
    if (manufacturerSlug === "best") {
        if (!(0, bestPricebook_1.isBestResidentialWorkbook)(filePath)) {
            throw new Error("Best preview currently expects the extracted Residential Price List workbook. Select the spreadsheet child file from the archive.");
        }
        return (0, bestPricebook_1.parseBestPricebookWorkbook)(filePath);
    }
    if (manufacturerSlug === "england") {
        return (0, englandPricebook_1.parseEnglandPricebookPdf)(filePath, input.execFileAsync);
    }
    if (manufacturerSlug === "jackson-catnapper") {
        return (0, jacksonCatnapperPricebook_1.parseJacksonCatnapperPricebookPdf)(filePath, input.execFileAsync);
    }
    if (manufacturerSlug === "guardsman") {
        return (0, accessoryWorkbookPricebooks_1.parseGuardsmanWorkbook)(filePath);
    }
    if (manufacturerSlug === "gbs-protectall") {
        return (0, accessoryWorkbookPricebooks_1.parseGbsProtectallWorkbook)(filePath);
    }
    if (manufacturerSlug === "innovations") {
        return (0, accessoryWorkbookPricebooks_1.parseInnovationsWorkbook)(filePath);
    }
    throw new Error(`No parser is available yet for ${resolvedUploadRow.manufacturer}. Liberty, Best, England, Jackson/Catnapper, Guardsman, GBS ProtectAll, and Innovations are currently live.`);
}
async function parseUploadReferenceNotes(input) {
    ...
    if (!fs_1.default.existsSync(filePath))
        return [];
    const manufacturerSlug = String(resolvedUploadRow.manufacturer_slug || "").trim().toLowerCase();
    if (String(resolvedUploadRow.document_type || "pricebook") === "archive")
        return [];
    if (manufacturerSlug === "liberty") {
        return (0, libertyPricebook_1.parseLibertyReferenceNotesFromPdf)(filePath, input.execFileAsync);
    }
    if (manufacturerSlug === "best") {
        if (!(0, bestPricebook_1.isBestResidentialWorkbook)(filePath))
            return [];
        return (0, bestPricebook_1.parseBestReferenceNotes)(filePath);
    }
    if (manufacturerSlug === "england") {
        return (0, englandPricebook_1.parseEnglandReferenceNotes)();
    }
    if (manufacturerSlug === "jackson-catnapper") {
        return (0, jacksonCatnapperPricebook_1.parseJacksonCatnapperReferenceNotes)();
    }
    if (manufacturerSlug === "guardsman" || manufacturerSlug === "gbs-protectall" || manufacturerSlug === "innovations") {
        return (0, accessoryWorkbookPricebooks_1.parseAccessoryReferenceNotes)();
    }
    return [];
}
function normalizeDraftRows(rows, uploadRow) {
...
async function replaceCatalogForUpload(client, uploadRow, rows) {
...
function registerManufacturerPricebookRoutes({ app, pool, requireOwner, holdingDir, execFileAsync, imageCandidateRuns = DEFAULT_IMAGE_CANDIDATE_RUNS, }) {
    const upload = (0, multer_1.default)({
        ...
        fileFilter: (_req, file, cb) => {
            ...
            cb((ok ? null : new Error("Only PDF, CSV, XLS, XLSX, and ZIP files are accepted")), ok);
        },
    ...
    app.get("/api/manufacturer-pricebooks/summary", requireOwner, async (_req, res) => {
        ...
        const manufacturers = [...bySlug.values()].sort((left, right) => String(left.manufacturer).localeCompare(String(right.manufacturer)));
        res.json({
            ok: true,
            totals: {
    ...
    app.get("/api/manufacturer-pricebooks/uploads", requireOwner, async (req, res) => {
        const manufacturer = typeof req.query?.manufacturer === "string" ? String(req.query.manufacturer).trim() : "";
        const values = [];
        const where = [];
        if (manufacturer) {
    ...
    app.post("/api/manufacturer-pricebooks/uploads", requireOwner, upload.any(), async (req, res) => {
        const requestFiles = Array.isArray(req.files)
            ? req.files
            : [];
        if (!requestFiles.length)
            return res.status(400).json({ ok: false, error: "No files uploaded" });
        const cleanupTempFile = (fileName) => {
            const tempPath = path_1.default.join(holdingDir, fileName);
        ...
        const manufacturer = typeof req.body?.manufacturer === "string" ? String(req.body.manufacturer).trim() : "";
        if (!manufacturer) {
        ...
        const replaceExisting = req.body?.replace_existing === undefined
            ? true
            : String(req.body.replace_existing).trim().toLowerCase() !== "false";
        const manufacturerDir = path_1.default.join(holdingDir, manufacturerSlug);
        fs_1.default.mkdirSync(manufacturerDir, { recursive: true });
        ...
        for (const rawFile of requestFiles) {
            ...
            const sourcePath = path_1.default.join(holdingDir, rawFile.filename);
            const targetPath = path_1.default.join(manufacturerDir, storageName);
            fs_1.default.renameSync(sourcePath, targetPath);
            const relativePath = path_1.default.join(manufacturerSlug, storageName).replace(/\\/g, "/");
    ...
    app.get("/api/manufacturer-pricebooks/uploads/:uploadId/analyze", requireOwner, async (req, res) => {
        ...
        try {
            ...
            const filePath = path_1.default.join(holdingDir, String(resolvedUploadRow.relative_path || ""));
            if (!fs_1.default.existsSync(filePath)) {
    ...
    app.post("/api/manufacturer-pricebooks/uploads/:uploadId/mapped-preview", requireOwner, async (req, res) => {
        ...
        `, [Number(resolvedUploadRow.id), built.rows.length]);
            res.json({
                ...
                notes: [],
                rows: built.rows.map((row, index) => mapCatalogRow({
                    ...row,
    ...
    app.get("/api/manufacturer-pricebooks/uploads/:uploadId/preview", requireOwner, async (req, res) => {
        ...
        `, [Number(uploadRow.id), rows.length]);
            res.json({
                ...
                notes,
                rows: rows.map((row, index) => mapCatalogRow({
                    ...row,
                    id: `preview-${index + 1}`,
    ...
    app.post("/api/manufacturer-pricebooks/uploads/:uploadId/publish", requireOwner, async (req, res) => {
        ...
        try {
            ...
            const rows = draftRows.length > 0 ? draftRows : await parseUploadRows({ pool, holdingDir, uploadRow, execFileAsync });
            const notes = await parseUploadReferenceNotes({ pool, holdingDir, uploadRow, execFileAsync });
            if (!rows.length) {
          ...
          `, [Number(uploadRow.id), rows.length]);
            ...
            const countResult = await pool.query(`SELECT COUNT(*)::int AS n FROM manufacturer_catalog_items WHERE manufacturer_slug = $1`, [String(uploadRow.manufacturer_slug || "")]);
            res.json({
                ok: true,
    ...
    app.get("/api/shop/kiosk/products", async (req, res) => {
        ...
        if (query) {
            ...
            where.push(`(lower(search_text) LIKE '%' || $${values.length} || '%' OR lower(sku) LIKE '%' || $${values.length} || '%')`);
        }
    ...
    app.post("/api/shop/kiosk/products/:productId/image-candidates/:candidateId/approve", requireKioskImageApproval, async (req, res) => {
        ...
        if (!productId || !candidateId)
            return res.status(400).json({ ok: false, error: "product and candidate are required" });
        const candidate = loadKioskImageCandidates(imageCandidateRuns, new Set([productId])).find((row) => row.id === candidateId);
        if (!candidate)
            return res.status(404).json({ ok: false, error: "image candidate not found" });
...
//# sourceMappingURL=manufacturerPricebookRoutes.js.map