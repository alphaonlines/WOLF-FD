...
const crmWeather_1 = require("../crmWeather");
const UPS_LANES = ["Unattended", "Be-Back", "Quote Follow-up"];
const UPS_PRIORITIES = ["Hot", "Today", "Nurture"];
const UPS_QUEUE_CUSTOMER_TYPES = ["Regular Up", "B-Back"];
const UPS_QUEUE_STATUSES = ["waiting", "working", "on_break"];
...
`;
function parseUpsLane(value) {
    if (!value || typeof value !== "string")
        return null;
    const lane = value.trim();
    return UPS_LANES.includes(lane) ? lane : null;
}
function parseUpsPriority(value) {
    if (!value || typeof value !== "string")
        return null;
    const priority = value.trim();
    return UPS_PRIORITIES.includes(priority) ? priority : null;
}
function parseUpsQueueCustomerType(value) {
    if (!value || typeof value !== "string")
        return null;
    const type = value.trim();
    return UPS_QUEUE_CUSTOMER_TYPES.includes(type) ? type : null;
}
function parseUpsQueueStatus(value) {
    if (!value || typeof value !== "string")
        return null;
    const status = value.trim();
    return UPS_QUEUE_STATUSES.includes(status) ? status : null;
}
function buildUpsHistoryId() {
...
function isSalesOnly(user) {
    return hasAnyRole(user, ["Sales"]) && !isManagerOrOwner(user);
}
function normalizePersonNameTokens(value) {
    return String(value || "")
        .toUpperCase()
        .split(/[^A-Z0-9]+/)
...
function canManageUpsQueueRow(user, row) {
    return Boolean(user);
}
async function resolveOwner(pool, ownerUserIdRaw, ownerRaw) {
    const ownerUserId = ownerUserIdRaw === null || ownerUserIdRaw === undefined || ownerUserIdRaw === ""
        ? null
        : Number(ownerUserIdRaw);
    if (ownerUserId !== null && Number.isFinite(ownerUserId) && ownerUserId > 0) {
        const userRow = await pool.query("SELECT id, name, email FROM users WHERE id = $1 AND active = TRUE LIMIT 1", [ownerUserId]);
...
function mapUpsQueueRow(row) {
    return {
        ...
        current_weather_temp_f: row.current_weather_temp_f === null || row.current_weather_temp_f === undefined
            ? null
            : Number(row.current_weather_temp_f),
        current_weather_precip_pct: row.current_weather_precip_pct === null || row.current_weather_precip_pct === undefined
            ? null
            : Number(row.current_weather_precip_pct),
        current_weather_wind_mph: row.current_weather_wind_mph === null || row.current_weather_wind_mph === undefined
            ? null
        ...
        live_weather_temp_f: row.live_weather_temp_f === null || row.live_weather_temp_f === undefined ? null : Number(row.live_weather_temp_f),
        live_weather_precip_pct: row.live_weather_precip_pct === null || row.live_weather_precip_pct === undefined ? null : Number(row.live_weather_precip_pct),
        live_weather_wind_mph: row.live_weather_wind_mph === null || row.live_weather_wind_mph === undefined ? null : Number(row.live_weather_wind_mph),
        ...
        active_customer_count: row.active_customer_count === null || row.active_customer_count === undefined ? 0 : Number(row.active_customer_count),
        active_customers: Array.isArray(row.active_customers)
            ? row.active_customers.map((entry) => ({
                id: String(entry?.id ?? ""),
                ...
                did_purchase: entry?.did_purchase === null || entry?.did_purchase === undefined ? null : Boolean(entry.did_purchase),
                purchase_amount: entry?.purchase_amount === null || entry?.purchase_amount === undefined ? null : Number(entry.purchase_amount),
...
async function fetchUpsQueueRows(pool, options) {
    const values = [];
    const where = [];
    if (options?.store) {
...
async function syncUpsQueueRowFromActiveCustomers(pool, queueEntryId) {
...
function buildQueueRepMetricKey(row) {
...
async function reorderUpsQueueStore(pool, store) {
...
function buildLooseSearchClause(columns, query, phoneDigits) {
    const values = [];
    const conditions = [];
...
function registerCrmRoutes(app, pool) {
    ...
    app.patch("/api/crm/leads/:id/assign", async (req, res) => {
        ...
        const salesOnly = isSalesOnly(user);
        const currentOwnerUserId = existing.rows[0].owner_user_id === null || existing.rows[0].owner_user_id === undefined
            ? null
    ...
    app.patch("/api/crm/leads/:id", async (req, res) => {
        ...
        if (isSalesOnly(user)) {
            ...
            if (!own.rows.length)
                return res.status(404).json({ error: "not found" });
            const ownerUserId = own.rows[0].owner_user_id === null || own.rows[0].owner_user_id === undefined
                ? null
    ...
    app.post("/api/crm/ups", async (req, res) => {
        ...
        if (!user)
            return res.status(401).json({ error: "unauthorized" });
        const id = (0, parsers_1.parseCrmLeadId)(req.body?.id) ?? `ups-${Date.now()}`;
        ...
        const dueAt = (0, parsers_1.parseCrmDate)(req.body?.due_at) ?? new Date().toISOString().slice(0, 10);
        const channel = (0, parsers_1.parseCrmChannel)(req.body?.channel) ?? "SMS";
        const done = (0, parsers_1.parseCrmBool)(req.body?.done) ?? false;
    ...
    app.patch("/api/crm/ups/:id", async (req, res) => {
        ...
        if (!current.rows.length)
            return res.status(404).json({ error: "not found" });
        if (isSalesOnly(user)) {
            const currentOwnerUserId = current.rows[0].owner_user_id === null || current.rows[0].owner_user_id === undefined
                ? null
        ...
        if (req.body?.due_at !== undefined) {
            const dueAt = req.body?.due_at === "" ? null : (0, parsers_1.parseCrmDate)(req.body?.due_at);
        ...
        if (req.body?.done !== undefined) {
            const done = (0, parsers_1.parseCrmBool)(req.body?.done);
            if (done === null)
                return res.status(400).json({ error: "invalid done" });
            values.push(done);
            fields.push(`done = $${values.length}`);
        }
        if (req.body?.started_at !== undefined) {
            const startedAt = req.body?.started_at === null || req.body?.started_at === ""
                ? null
    ...
    app.get("/api/crm/ups-history", async (req, res) => {
        ...
        const reportDate = (0, parsers_1.parseCrmDate)(dateRaw) || new Date().toISOString().slice(0, 10);
    ...
    app.post("/api/crm/ups-queue", async (req, res) => {
      ...
      `, [repUserId]);
            ...
            const roles = Array.isArray(matchedUser.rows[0].roles)
                ? matchedUser.rows[0].roles.map((role) => String(role))
                : [];
            if (!roles.some((role) => ["Owner", "Manager", "Sales"].includes(role))) {
        ...
        const id = (0, parsers_1.parseCrmLeadId)(req.body?.id) ?? `ups-rep-${Date.now()}`;
        await pool.query(`
    ...
    app.post("/api/crm/ups-queue/:id/start", async (req, res) => {
        ...
        const weather = await (0, crmWeather_1.getStoreWeatherSnapshot)(store);
    ...
    app.patch("/api/crm/ups-queue/:id/customers/:customerId", async (req, res) => {
        ...
        if (!user)
            return res.status(401).json({ error: "unauthorized" });
        const id = (0, parsers_1.parseCrmLeadId)(req.params.id);
        const customerId = (0, parsers_1.parseCrmLeadId)(req.params.customerId);
        if (!id)
            return res.status(400).json({ error: "invalid id" });
        if (!customerId)
        ...
        const activeRow = await pool.query(`SELECT id, history_id FROM crm_ups_active_customers WHERE id = $1 AND queue_entry_id = $2 LIMIT 1`, [customerId, id]);
        ...
        if (req.body?.did_purchase !== undefined) {
            const didPurchase = (0, parsers_1.parseCrmBool)(req.body.did_purchase);
            if (didPurchase === null)
                return res.status(400).json({ error: "invalid did_purchase" });
    ...
    app.post("/api/crm/ups-queue/:id/customers/:customerId/complete", async (req, res) => {
        ...
        const activeCustomer = await pool.query(`SELECT id, history_id FROM crm_ups_active_customers WHERE id = $1 AND queue_entry_id = $2 LIMIT 1`, [customerId, id]);
        if (!activeCustomer.rows.length)
            return res.status(404).json({ error: "active customer not found" });
    ...
    `, [store, status]);
        const ids = peers.rows.map((entry) => String(entry.id));
    ...
    app.delete("/api/crm/ups-queue/:id", async (req, res) => {
        ...
        try {
            await client.query("BEGIN");
            const activeHistoryRows = await client.query(`SELECT history_id FROM crm_ups_active_customers WHERE queue_entry_id = $1 AND history_id IS NOT NULL`, [id]);
            const historyIds = activeHistoryRows.rows
                .map((entry) => String(entry.history_id || ""))
                .filter(Boolean);
    ...
    `, [phoneNorm]);
        ...
        const lifetimeRes = phoneNorm
          ...
          WHERE regexp_replace(COALESCE(phone, ''), '\\D', '', 'g') LIKE ('%' || $1 || '%')
            AND grand_total IS NOT NULL`, [phoneNorm])
    ...
    app.get("/api/crm/customers/visits-by-name", async (req, res) => {
        ...
        const countRes = await pool.query(`SELECT COUNT(*)::int AS visit_count FROM crm_ups_history WHERE ${conditions}`, values);
        const recentRes = await pool.query(`SELECT id, store, rep, customer, customer_type, started_at, completed_at, did_purchase, purchase_amount
       FROM crm_ups_history
       WHERE ${conditions}
    ...
    app.get("/api/crm/search", async (req, res) => {
        ...
        const customerSearch = buildLooseSearchClause(["name", "email", "store", "notes", "source", "interest", "owner", "stage", "last_message", "next_action"], queryRaw, phoneDigits);
        const orderSearch = buildLooseSearchClause(["customer_name", "receipt_no", "location", "salesperson"], queryRaw, phoneDigits);
    ...
    app.post("/api/crm/customers/upsert", async (req, res) => {
        ...
        const stage = (0, parsers_1.parseCrmStage)(req.body?.stage) ?? "New";
        const nextAction = typeof req.body?.next_action === "string" ? req.body.next_action.trim() : "";
        const dueDate = req.body?.due_date ? (0, parsers_1.parseCrmDate)(req.body?.due_date) : null;
        const lastMessage = typeof req.body?.last_message === "string" ? req.body.last_message.trim() : "";
        const lastTouch = typeof req.body?.last_touch === "string" ? req.body.last_touch.trim() : "";
        const notes = typeof req.body?.notes === "string" ? req.body.notes.trim() : "";
        if (!name)
...
//# sourceMappingURL=crmRoutesV2.js.map