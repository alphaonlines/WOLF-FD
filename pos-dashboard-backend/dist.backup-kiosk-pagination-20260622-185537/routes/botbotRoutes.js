...
exports.registerBotBotRoutes = registerBotBotRoutes;
const llmClient_1 = require("../llmClient");
const botbotAccess_1 = require("../botbotAccess");
const botbotPrompt_1 = require("../botbotPrompt");
...
const pct = (value) => value === null || !Number.isFinite(value) ? "unknown" : `${value.toFixed(1)}%`;
const asNumber = (value) => {
    const numeric = Number(value);
    return Number.isFinite(numeric) ? numeric : 0;
};
const arrayLength = (value) => (Array.isArray(value) ? value.length : 0);
const isSalesContext = (ctx) => ctx.pageId === "sales-dashboard" || ctx.subPageId === "pulse-sales" || ctx.module === "sales";
const getContextFilter = (ctx, key) => {
    const value = ctx.filters?.[key];
    return value === null || value === undefined || String(value).trim() === "" ? null : String(value).trim();
};
const salesContextParams = (ctx) => {
...
async function buildSalesSnapshot(pool, params) {
    const pro1stItemSql = (0, pro1stSql_1.buildQualifiedPro1stSql)("i.");
    const excludedPro1stSql = (0, pro1stSql_1.buildPro1stExcludedSql)("i.");
  ...
  `;
    ...
    return {
        ...
        lowMargin: {
            ...
            topTickets: lowMarginResult.rows.map((row) => ({
...
function formatSalesSnapshotForPrompt(snapshot) {
    const filterParts = [
...
const parseQuota = (value, fallback) => {
    const numeric = Number(value);
    return Number.isFinite(numeric) && numeric >= 0 ? Math.round(numeric) : fallback;
};
const tokenLedgerKeyForProvider = (provider, modelKey) => (0, botbotAccess_1.isLocalProvider)(provider) ? "local" : modelKey;
const normalizeExternalUserKey = (value) => String(value || "")
    .trim()
    .toLowerCase()
    .slice(0, 160);
async function findExternalBotBotUser(pool, userKey) {
    const userResult = await pool.query(`SELECT u.id, u.name, u.email,
...
async function buildExternalBotBotUsage(pool, userRow) {
    const user = {
        id: String(userRow.id),
        name: String(userRow.name || ""),
        roles: Array.isArray(userRow.roles) ? userRow.roles.map((role) => String(role)) : [],
    };
    const result = await pool.query(`SELECT m.model_key, m.display_name, m.provider, m.free_token_quota AS quota,
     ...
     ORDER BY m.sort_order ASC`, [Number(userRow.id)]);
    const usage = [];
    for (const row of result.rows) {
        const access = await (0, botbotAccess_1.resolveModelAccess)(pool, user, String(row.model_key), String(row.provider ?? ""), parseQuota(row.quota, 0));
        if (!access.allowed)
            continue;
...
const clipPromptText = (value, maxLength) => typeof value === "string" ? value.trim().slice(0, maxLength) : "";
function buildPromptContextAddendum(promptContext) {
...
async function loadObjectionHandlingPromptContext(pool) {
    try {
        const result = await pool.query(`SELECT label, rebuttals
...
async function logBotBotUsageEvent(pool, event) {
    ...
    await pool.query(`INSERT INTO botbot_usage_events
...
async function loadSubjectAccess(pool, subjectType, subjectKey) {
    const [modelRows, skillRows, modelsResult, skillsResult] = await Promise.all([
        pool.query(`SELECT model_key, allowed, token_quota
       FROM botbot_model_access
       WHERE subject_type = $1 AND subject_key = $2`, [subjectType, subjectKey]),
        pool.query(`SELECT skill_key, allowed
       FROM botbot_skill_access
       WHERE subject_type = $1 AND subject_key = $2`, [subjectType, subjectKey]),
        pool.query(`SELECT model_key, display_name, provider, free_token_quota, enabled, sort_order
       FROM botbot_model_config
       ORDER BY sort_order ASC`),
        pool.query(`SELECT skill_key, label, description, default_allowed, admin_only
...
async function saveSubjectAccess(pool, subjectType, subjectKey, body) {
    ...
    for (const [modelKey, raw] of Object.entries(models)) {
        ...
        await pool.query(`INSERT INTO botbot_model_access
    ...
    for (const [skillKey, allowed] of Object.entries(skills)) {
        await pool.query(`INSERT INTO botbot_skill_access
         (subject_type, subject_key, skill_key, allowed, created_at, updated_at)
       VALUES ($1, $2, $3, $4, now(), now())
       ON CONFLICT (subject_type, subject_key, skill_key)
       DO UPDATE SET allowed = EXCLUDED.allowed,
                     updated_at = now()`, [subjectType, subjectKey, String(skillKey), Boolean(allowed)]);
    }
}
function registerBotBotRoutes({ app, pool, requireOwner, }) {
    const getAuthUser = (req) => req.authUser;
    const userId = (req) => parseInt(getAuthUser(req).id, 10);
    app.post("/api/botbot/external/usage", async (req, res) => {
        const headerToken = String(req.headers["x-botbot-ledger-token"] || "");
        if (!runtimeConfig_1.BOTBOT_LEDGER_TOKEN || headerToken !== runtimeConfig_1.BOTBOT_LEDGER_TOKEN) {
        ...
        if (totalTokens > 0) {
            await pool.query(`INSERT INTO botbot_token_ledger (user_id, model_key, tokens_used, updated_at)
         VALUES ($1, $2, $3, now())
         ON CONFLICT (user_id, model_key)
         DO UPDATE SET
           tokens_used = botbot_token_ledger.tokens_used + EXCLUDED.tokens_used,
           updated_at = now()`, [Number(userRow.id), ledgerModelKey, totalTokens]);
        }
    ...
    app.get("/api/botbot/models", async (req, res) => {
        ...
        const r = await pool.query(`SELECT model_key, display_name, provider, ollama_model_name, free_token_quota, sort_order
       FROM botbot_model_config
       WHERE enabled = TRUE
       ORDER BY sort_order ASC`);
        const models = [];
        for (const row of r.rows) {
            const access = await (0, botbotAccess_1.resolveModelAccess)(pool, user, String(row.model_key), String(row.provider), parseQuota(row.free_token_quota, 0));
            if (!access.allowed)
                continue;
            models.push({
                modelKey: String(row.model_key),
    ...
    app.get("/api/botbot/runtime", async (req, res) => {
        const tagInfo = await fetchOllamaTags(runtimeConfig_1.OLLAMA_BASE_URL);
        res.json({
            runtime: {
                enabled: runtimeConfig_1.BOTBOT_ENABLED,
                endpointKey: "alphaai",
                endpointLabel: "AlphaAI model endpoint",
                endpointUrl: runtimeConfig_1.OLLAMA_BASE_URL,
                localAiPlatformUrl: runtimeConfig_1.BOTBOT_LOCAL_AI_URL,
                primaryNodeLabel: runtimeConfig_1.OLLAMA_PRIMARY_NODE_LABEL,
                primaryModel: runtimeConfig_1.OLLAMA_PRIMARY_MODEL,
                ...
                nodes: [
                    {
                        key: "alphaai",
                        label: "AlphaAI",
                        host: runtimeConfig_1.OLLAMA_PRIMARY_NODE_LABEL,
                        baseUrl: runtimeConfig_1.OLLAMA_BASE_URL,
    ...
    app.get("/api/botbot/context/sales-snapshot", async (req, res) => {
        ...
        const start = (0, parsers_1.parseDateParam)(req.query.start, today);
        const end = (0, parsers_1.parseDateParam)(req.query.end, tomorrow);
        const location = (0, parsers_1.parseTextParam)(req.query.location);
        const salesperson = (0, parsers_1.parseTextParam)(req.query.salesperson);
        const label = (0, parsers_1.parseTextParam)(req.query.label) || `${start} to ${end}`;
        try {
    ...
    app.get("/api/botbot/conversations", async (req, res) => {
        ...
        const r = await pool.query(`SELECT c.id, c.title, c.model_key, c.context_tag, c.updated_at,
    ...
    app.post("/api/botbot/conversations", async (req, res) => {
        ...
        const modelCheck = await pool.query(`SELECT model_key, provider, free_token_quota FROM botbot_model_config WHERE model_key = $1 AND enabled = TRUE`, [modelKey]);
        if (modelCheck.rows.length === 0) {
            return res.status(400).json({ ok: false, error: "invalid_model" });
        }
        const modelAccess = await (0, botbotAccess_1.resolveModelAccess)(pool, toAuthUser(getAuthUser(req)), modelKey, String(modelCheck.rows[0].provider ?? ""), parseQuota(modelCheck.rows[0].free_token_quota, 0));
        if (!modelAccess.allowed) {
            return res.status(403).json({ ok: false, error: "model_not_allowed" });
        }
        const r = await pool.query(`INSERT INTO botbot_conversations (user_id, title, model_key, context_tag)
       VALUES ($1, $2, $3, $4)
       RETURNING id, title, model_key, context_tag, created_at, updated_at`, [uid, title, modelKey, contextTag]);
    ...
    app.patch("/api/botbot/conversations/:id", async (req, res) => {
        ...
        const r = await pool.query(`UPDATE botbot_conversations
       SET title = $1, updated_at = now()
       WHERE id = $2 AND user_id = $3
       RETURNING id, title`, [title.slice(0, 100), convId, uid]);
        if (r.rows.length === 0)
    ...
    app.delete("/api/botbot/conversations/:id", async (req, res) => {
        const uid = userId(req);
        const convId = parseInt(req.params.id, 10);
        const r = await pool.query(`DELETE FROM botbot_conversations WHERE id = $1 AND user_id = $2 RETURNING id`, [convId, uid]);
        if (r.rows.length === 0)
            return res.status(404).json({ ok: false, error: "not_found" });
        res.json({ ok: true });
    });
    app.get("/api/botbot/conversations/:id/messages", async (req, res) => {
        const uid = userId(req);
        const convId = parseInt(req.params.id, 10);
        const ownerCheck = await pool.query(`SELECT id FROM botbot_conversations WHERE id = $1 AND user_id = $2`, [convId, uid]);
        if (ownerCheck.rows.length === 0)
            return res.status(404).json({ ok: false, error: "not_found" });
        const r = await pool.query(`SELECT id, role, content, model_key, input_tokens, output_tokens, finish_reason, created_at
       FROM botbot_messages
       WHERE conversation_id = $1
       ORDER BY id ASC
       LIMIT $2`, [convId, MAX_RESPONSE_MESSAGES]);
    ...
    app.post("/api/botbot/conversations/:id/messages", async (req, res) => {
        ...
        const convId = parseInt(req.params.id, 10);
        const { content, pageContext, promptContext } = (req.body ?? {});
        ...
        const convResult = await pool.query(`SELECT id, model_key, context_tag FROM botbot_conversations WHERE id = $1 AND user_id = $2`, [convId, uid]);
        ...
        const modelResult = await pool.query(`SELECT model_key, display_name, provider, ollama_model_name, free_token_quota, enabled
       FROM botbot_model_config WHERE model_key = $1`, [conv.model_key]);
        ...
        const modelAccess = await (0, botbotAccess_1.resolveModelAccess)(pool, toAuthUser(user), conv.model_key, provider, parseQuota(model.free_token_quota, 0));
        if (!modelAccess.allowed) {
            await logBotBotUsageEvent(pool, {
                userId: uid,
        ...
        const ledgerResult = await pool.query(`SELECT COALESCE(tokens_used, 0) AS tokens_used,
              COALESCE(tokens_purchased, 0) AS tokens_purchased
       FROM botbot_token_ledger
       WHERE user_id = $1 AND model_key = $2`, [uid, ledgerModelKey]);
        ...
        await pool.query(`INSERT INTO botbot_messages (conversation_id, role, content, model_key, input_tokens, output_tokens)
       VALUES ($1, 'user', $2, $3, 0, 0)`, [convId, content.trim(), conv.model_key]);
        const historyResult = await pool.query(`SELECT role, content FROM botbot_messages
       WHERE conversation_id = $1
       ORDER BY id DESC
       LIMIT $2`, [convId, HISTORY_LIMIT]);
        const history = historyResult.rows.reverse();
        const settingsResult = await pool.query(`SELECT assistant_name FROM botbot_settings WHERE user_id = $1`, [uid]);
        ...
        const skillKey = (0, botbotAccess_1.inferBotBotSkill)(ctx);
        const skillAccess = await (0, botbotAccess_1.resolveSkillAccess)(pool, toAuthUser(user), skillKey);
        if (!skillAccess.allowed) {
            const errMsg = "I can't help with that area yet because this BotBot skill is not enabled for your account.";
            const errRow = await pool.query(`INSERT INTO botbot_messages
           (conversation_id, role, content, model_key, input_tokens, output_tokens, finish_reason)
         VALUES ($1, 'assistant', $2, $3, 0, 0, 'denied')
         RETURNING id, role, content, model_key, input_tokens, output_tokens, finish_reason, created_at`, [convId, errMsg, conv.model_key]);
            await logBotBotUsageEvent(pool, {
                userId: uid,
                conversationId: convId,
        ...
        const objectionContextAddendum = promptContext?.includeObjections ? await loadObjectionHandlingPromptContext(pool) : "";
        const systemPrompt = [
            (0, botbotPrompt_1.buildSystemPrompt)(user.name, assistantName, ctx, liveContextSnapshot),
        ...
        try {
            if (model.provider === "wolfbot") {
                llmResponse = await (0, llmClient_1.callBotBotLocalAi)(model.ollama_model_name || runtimeConfig_1.OLLAMA_PRIMARY_MODEL, history, systemPrompt);
            }
            else if (model.provider === "ollama") {
                llmResponse = await (0, llmClient_1.callOllama)(model.ollama_model_name, history, systemPrompt, runtimeConfig_1.OLLAMA_BASE_URL);
            }
            else if (model.provider === "openai") {
                llmResponse = await (0, llmClient_1.callOpenAI)(model.ollama_model_name || conv.model_key, history, systemPrompt);
            }
            else if (model.provider === "anthropic") {
                llmResponse = await (0, llmClient_1.callClaude)(conv.model_key, history, systemPrompt);
            }
            else {
        ...
        const msgResult = await pool.query(`INSERT INTO botbot_messages
         (conversation_id, role, content, model_key, input_tokens, output_tokens, finish_reason)
       ...
       DO UPDATE SET
         tokens_used = botbot_token_ledger.tokens_used + EXCLUDED.tokens_used,
         updated_at = now()`, [uid, ledgerModelKey, totalNew]);
        await pool.query(`UPDATE botbot_conversations SET updated_at = now() WHERE id = $1`, [convId]);
    ...
    app.get("/api/botbot/token-usage", async (req, res) => {
        const uid = userId(req);
        const user = toAuthUser(getAuthUser(req));
        const r = await pool.query(`SELECT m.model_key, m.display_name, m.provider, m.free_token_quota AS quota,
              CASE WHEN m.provider IN ('wolfbot', 'ollama') THEN 'local' ELSE m.model_key END AS billing_model_key,
              COALESCE(l.tokens_used, 0) AS tokens_used,
              COALESCE(l.tokens_purchased, 0) AS tokens_purchased
       FROM botbot_model_config m
    ...
    app.get("/api/botbot/settings", async (req, res) => {
        const uid = userId(req);
        const r = await pool.query(`SELECT assistant_name, assistant_theme, tutorial_completed, preferred_model_key, preferred_runtime_node
       FROM botbot_settings WHERE user_id = $1`, [uid]);
        res.json({ settings: r.rows[0] ?? null });
    });
    app.put("/api/botbot/settings", async (req, res) => {
        const uid = userId(req);
        const { assistantName, assistantTheme, tutorialCompleted, preferredModelKey, } = (req.body ?? {});
        const validThemes = [
            "sky",
            "emerald",
            "violet",
        ...
        const safeTheme = validThemes.includes(assistantTheme)
            ? assistantTheme
            : undefined;
        await pool.query(`INSERT INTO botbot_settings (user_id, assistant_name, assistant_theme, tutorial_completed, preferred_model_key, preferred_runtime_node)
       VALUES ($1,
         COALESCE($2, 'BotBot'),
         COALESCE($3, 'sky'),
         COALESCE($4, FALSE),
       ...
       ON CONFLICT (user_id) DO UPDATE SET
         ...
         updated_at = now()`, [uid, safeName, safeTheme, tutorialCompleted, preferredModelKey, undefined]);
        res.json({ ok: true });
    });
    app.get("/api/botbot/admin/usage", requireOwner, async (req, res) => {
        ...
        const r = await pool.query(`SELECT u.id AS user_id, u.name AS user_name, u.email,
    ...
    app.get("/api/botbot/admin/model-config", requireOwner, async (_req, res) => {
        const r = await pool.query(`SELECT model_key, display_name, provider, ollama_model_name,
    ...
    app.get("/api/botbot/admin/usage/history", requireOwner, async (req, res) => {
        ...
        const r = await pool.query(`SELECT to_timestamp(floor(extract(epoch from created_at) / $2::numeric) * $2::numeric) AS bucket,
              COUNT(*)::int AS events,
       ...
       GROUP BY bucket
       ORDER BY bucket ASC`, [config.interval, config.bucketSeconds]);
        res.json({
    ...
    app.get("/api/botbot/admin/usage/by-skill", requireOwner, async (req, res) => {
        ...
        const r = await pool.query(`SELECT e.skill_key,
       ...
       ORDER BY total_tokens DESC, events DESC`, [config.interval]);
        res.json({
    ...
    app.get("/api/botbot/admin/skills", requireOwner, async (_req, res) => {
        const r = await pool.query(`SELECT skill_key, label, description, default_allowed, admin_only, updated_at
       FROM botbot_skill_catalog
       ORDER BY skill_key ASC`);
        res.json({
    ...
    app.patch("/api/botbot/admin/model-config/:modelKey", requireOwner, async (req, res) => {
        ...
        const r = await pool.query(`UPDATE botbot_model_config SET ${updates.join(", ")}
         WHERE model_key = $1
         RETURNING model_key, display_name, free_token_quota, enabled, sort_order`, values);
        if (r.rows.length === 0)
            return res.status(404).json({ ok: false, error: "not_found" });
    ...
    app.post("/api/botbot/admin/reset-user-quota", requireOwner, async (req, res) => {
        ...
        await pool.query(`UPDATE botbot_token_ledger
         SET tokens_used = 0, last_reset_at = now(), updated_at = now()
         WHERE user_id = $1 AND model_key = $2`, [parseInt(targetUserId, 10), modelKey]);
        res.json({ ok: true });
    });
}
//# sourceMappingURL=botbotRoutes.js.map