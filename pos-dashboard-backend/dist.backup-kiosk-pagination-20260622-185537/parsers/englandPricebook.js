...
exports.parseEnglandPricebookPdf = parseEnglandPricebookPdf;
exports.parseEnglandReferenceNotes = parseEnglandReferenceNotes;
...
function detectSection(pageLines) {
    const topText = pageLines.slice(0, 12).join(" ");
    if (/\bADDITIONAL ITEMS\b/i.test(topText))
        return "Additional Items";
    if (/\bLEATHER\b/i.test(topText))
        return "Leather";
    if (/\bSTATIONARY\b/i.test(topText))
        return "Stationary";
    return "";
}
function parseCollection(pageLines) {
...
function detectProductType(text, section) {
    ...
    if (value.includes("chair"))
        return "chair";
    if (value.includes("ottoman") || value.includes("otto"))
        return "ottoman";
    if (value.includes("pillow"))
        return "pillow";
    if (value.includes("fabric yardage") || value.includes("price per yard"))
        return "fabric";
    if (value.includes("hide"))
        return "leather hide";
    if (value.includes("mechanism"))
        return "mechanism";
    if (value.includes("console"))
        return "console";
    return section ? section.toLowerCase() : "";
}
function detectCategory(section, productType) {
    if (section === "Leather")
        return "Leather Upholstery";
    if (section === "Stationary")
        return "Stationary Upholstery";
    if (productType === "mechanism")
        return "Mechanisms";
...
function detectMaterial(section, text) {
    const value = text.toLowerCase();
    if (section === "Leather" || value.includes("leather"))
        return "leather";
    if (value.includes("fabric") || value.includes("pillow"))
        return "fabric";
    return "upholstery";
}
function detectFeatureTags(text, section) {
    const value = `${text} ${section}`.toLowerCase();
    const tags = new Set();
    [
        "laf",
...
function parseRow(line, section, collection, typeLine, sourceSortOrder) {
    ...
    if (!sku || !description || !/\d/.test(tail))
        return null;
    if (/^(features|backs|seats|legs|style)$/i.test(sku))
        return null;
    const firstPriceMatch = tail.match(/\d+(?:\.\d+)?/);
    const basePrice = parseNumber(firstPriceMatch?.[0] || null);
    if (basePrice === null)
        return null;
    const numericTokens = Array.from(tail.matchAll(/\d+(?:\.\d+)?/g), (entry) => Number(entry[0])).filter((value) => Number.isFinite(value));
    ...
    return {
        ...
        shape: "",
        dimensionsText: widthInches && depthInches && heightInches ? `W ${widthInches}" · D ${depthInches}" · H ${heightInches}"` : "",
        widthInches,
        depthInches,
        heightInches,
        cubes,
...
async function parseEnglandPricebookPdf(filePath, execFileAsync) {
    void execFileAsync;
...
async function parseEnglandReferenceNotes() {
    return [];
}
//# sourceMappingURL=englandPricebook.js.map