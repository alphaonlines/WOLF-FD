...
exports.runStartupBootstrap = runStartupBootstrap;
...
async function ensureAuthSchema(pool) {
  ...
  `);
    ...
    await pool.query(`ALTER TABLE users ADD COLUMN IF NOT EXISTS tutorial_reset_at TIMESTAMPTZ;`);
...
async function ensureDefaultRolePermissions(pool) {
    ...
    for (const role of roleRows) {
        ...
        if (!Number.isFinite(roleId) || !roleKey)
            continue;
        const defaults = new Set((0, permissionCatalog_1.getRoleDefaultPermissionKeys)(roleKey));
        for (const permission of permissionCatalog_1.PERMISSION_CATALOG) {
...
async function ensureDefaultAuthUser({ pool, envString, hashPassword, setUserRolesByKeys, }) {
    ...
    if (count <= 0) {
        ...
        await pool.query(`INSERT INTO users (name, email, password_hash, active, created_at, updated_at)
       VALUES ($1, $2, $3, TRUE, now(), now())
       ON CONFLICT (email) DO NOTHING;`, [defaultName, defaultEmail, passwordHash]);
...
async function ensureSocialSchema(pool) {
    await pool.query(`
...
async function ensureWebTrackingSchema(pool) {
    await pool.query(`
...
async function ensureManufacturerPricebookSchema(pool) {
    await pool.query(`
  ...
  `);
    ...
    await pool.query(`CREATE INDEX IF NOT EXISTS idx_manufacturer_pricebook_uploads_manufacturer_created ON manufacturer_pricebook_uploads(manufacturer, created_at DESC);`);
    await pool.query(`CREATE INDEX IF NOT EXISTS idx_manufacturer_pricebook_uploads_status_created ON manufacturer_pricebook_uploads(status, created_at DESC);`);
    await pool.query(`
  ...
  `);
    ...
    await pool.query(`CREATE INDEX IF NOT EXISTS idx_manufacturer_catalog_items_lookup ON manufacturer_catalog_items(manufacturer_slug, category, color_family, created_at DESC);`);
    await pool.query(`CREATE INDEX IF NOT EXISTS idx_manufacturer_catalog_items_upload_sort ON manufacturer_catalog_items(upload_id, source_sort_order);`);
    await pool.query(`CREATE INDEX IF NOT EXISTS idx_manufacturer_catalog_items_search_text ON manufacturer_catalog_items USING GIN (to_tsvector('simple', search_text));`);
    await pool.query(`
  ...
  `);
    ...
    await pool.query(`CREATE INDEX IF NOT EXISTS idx_manufacturer_reference_notes_lookup ON manufacturer_reference_notes(manufacturer_slug, note_type, source_sort_order);`);
    await pool.query(`
  ...
  `);
    await pool.query(`ALTER TABLE manufacturer_pricebook_mapping_profiles ADD COLUMN IF NOT EXISTS manufacturer TEXT;`);
    await pool.query(`ALTER TABLE manufacturer_pricebook_mapping_profiles ADD COLUMN IF NOT EXISTS manufacturer_slug TEXT;`);
    await pool.query(`ALTER TABLE manufacturer_pricebook_mapping_profiles ADD COLUMN IF NOT EXISTS profile_name TEXT;`);
    await pool.query(`ALTER TABLE manufacturer_pricebook_mapping_profiles ADD COLUMN IF NOT EXISTS file_type TEXT;`);
    await pool.query(`ALTER TABLE manufacturer_pricebook_mapping_profiles ADD COLUMN IF NOT EXISTS sheet_name TEXT;`);
    await pool.query(`ALTER TABLE manufacturer_pricebook_mapping_profiles ADD COLUMN IF NOT EXISTS header_row_index INTEGER;`);
    await pool.query(`ALTER TABLE manufacturer_pricebook_mapping_profiles ADD COLUMN IF NOT EXISTS mappings JSONB;`);
    await pool.query(`ALTER TABLE manufacturer_pricebook_mapping_profiles ADD COLUMN IF NOT EXISTS updated_by_user_id BIGINT;`);
    await pool.query(`ALTER TABLE manufacturer_pricebook_mapping_profiles ADD COLUMN IF NOT EXISTS created_at TIMESTAMPTZ;`);
    await pool.query(`ALTER TABLE manufacturer_pricebook_mapping_profiles ADD COLUMN IF NOT EXISTS updated_at TIMESTAMPTZ;`);
    await pool.query(`ALTER TABLE manufacturer_pricebook_mapping_profiles ALTER COLUMN profile_name SET DEFAULT 'Default';`);
    await pool.query(`ALTER TABLE manufacturer_pricebook_mapping_profiles ALTER COLUMN file_type SET DEFAULT '';`);
    await pool.query(`ALTER TABLE manufacturer_pricebook_mapping_profiles ALTER COLUMN sheet_name SET DEFAULT '';`);
    await pool.query(`ALTER TABLE manufacturer_pricebook_mapping_profiles ALTER COLUMN header_row_index SET DEFAULT 0;`);
    await pool.query(`ALTER TABLE manufacturer_pricebook_mapping_profiles ALTER COLUMN mappings SET DEFAULT '{}'::jsonb;`);
    await pool.query(`ALTER TABLE manufacturer_pricebook_mapping_profiles ALTER COLUMN created_at SET DEFAULT now();`);
    await pool.query(`ALTER TABLE manufacturer_pricebook_mapping_profiles ALTER COLUMN updated_at SET DEFAULT now();`);
    await pool.query(`CREATE UNIQUE INDEX IF NOT EXISTS idx_manufacturer_pricebook_mapping_profiles_unique ON manufacturer_pricebook_mapping_profiles(manufacturer_slug, profile_name);`);
}
async function ensureBotBotSchema(pool) {
    await pool.query(`
  ...
  `);
    ...
    for (const skill of botbotAccess_1.BOTBOT_SKILL_CATALOG) {
        await pool.query(`INSERT INTO botbot_skill_catalog
       ...
       ON CONFLICT (skill_key) DO UPDATE SET
         ...
         updated_at = now()`, [skill.skillKey, skill.label, skill.description, skill.defaultAllowed, Boolean(skill.adminOnly)]);
    }
    await pool.query(`
    CREATE TABLE IF NOT EXISTS botbot_settings (
      ...
      preferred_runtime_node TEXT NOT NULL DEFAULT '${runtimeConfig_1.DEFAULT_OLLAMA_NODE_KEY}',
      created_at          TIMESTAMPTZ NOT NULL DEFAULT now(),
      updated_at          TIMESTAMPTZ NOT NULL DEFAULT now()
    );
  `);
    ...
    await pool.query(`ALTER TABLE botbot_settings ALTER COLUMN preferred_runtime_node SET DEFAULT '${runtimeConfig_1.DEFAULT_OLLAMA_NODE_KEY}';`);
  ...
  `);
    ...
    VALUES
      ('local', 'AlphaAI Default', 'wolfbot', '${runtimeConfig_1.OLLAMA_PRIMARY_MODEL}', 500000, TRUE, 1),
      ...
      ('${runtimeConfig_1.OPENAI_FAST_MODEL}', 'OpenAI Fast API', 'openai', '${runtimeConfig_1.OPENAI_FAST_MODEL}', 50000, TRUE, 20),
      ('${runtimeConfig_1.OPENAI_BALANCED_MODEL}', 'OpenAI Balanced API', 'openai', '${runtimeConfig_1.OPENAI_BALANCED_MODEL}', 25000, TRUE, 21),
    ...
    ON CONFLICT (model_key) DO UPDATE SET
      ...
      ollama_model_name = CASE
        WHEN botbot_model_config.model_key IN ('local', 'gemma4:e4b-it-q4_K_M', 'gemma4:e2b-it-q4_K_M', 'qwen2.5-coder:7b', '${runtimeConfig_1.OPENAI_FAST_MODEL}', '${runtimeConfig_1.OPENAI_BALANCED_MODEL}') THEN EXCLUDED.ollama_model_name
...
async function runStartupBootstrap(deps) {
...
//# sourceMappingURL=startupBootstrap.js.map